...
Coded with no external dependencies, using only canvas API.
`}update(t){++this.frame;for(let e=0;e<this.gridHeight;++e)for(let t=0;t<this.gridWidth;++t){var i=this.numAliveInMooreNeighborhood(t,e),s=this.getIdx(t,e);2===i&&this.grid[s]<0?this.gridNext[s]=1:this.gridNext[s]=this.grid[s]-1}[this.grid,this.gridNext]=[this.gridNext,this.grid]}}e.exports=s},{"./game-of-life":10}],5:[function(t,e,i){var s=t("../animation");let a=t("../utils");class r extends s{constructor(t,e,i,s,a=400,r=1,h=.05,n=!1){super(t,e,i,s,"cardioids with a pencil of lines","cardioids.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.lines=a,this.scale=r,this.speed=h,this.rainbowColors=n,this.radius=0}getVec(t){t=a.remap(t,0,this.lines,0,2*Math.PI);return a.rotateVec2d(a.createVec2d(this.radius,0),Math.PI+t)}draw(){this.clear(),this.radius=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/3*this.scale,this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2),a.strokeCircle(this.ctx,0,0,this.radius,this.colors[0]);for(let e=0;e<=this.lines;++e){var i=this.getVec(e),s=this.getVec(e*this.time);let t;t=this.rainbowColors?"hsl("+e/this.lines*360+", 100%, 75%)":a.lerpColorsPallet([this.colorA,this.colorB,this.colorA],e/this.lines),this.ctx.strokeStyle=t,a.drawLine(this.ctx,i.x,i.y,s.x,s.y,1,t)}this.ctx.resetTransform()}getSettings(){return[{prop:"lines",type:"int",min:1,max:2500},{prop:"speed",type:"float",min:-2,max:2},{prop:"scale",type:"float",min:.25,max:1.75},{prop:"rainbowColors",name:'<i class="fa-solid fa-rainbow"></i> rainbow colors',type:"bool"}]}}e.exports=r},{"../animation":2,"../utils":36}],6:[function(t,e,i){var s=t("../animation");let h=t("../utils");class a extends s{constructor(t,e,i,s,a=180,r=.5,h=.4,n=1.2,o=.001,l=!1){super(t,e,i,s,"circular waves","circular-waves.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.vertices=a,this.noiseScale=r,this.radiusScaleMin=h,this.radiusScaleMax=n,this.fadingSpeed=o,this.rainbowColors=l,this.radiusMin=0,this.radiusMax=0}draw(){this.fadeOut(this.fadingSpeed);var e=.005*this.frame,i=2*Math.PI/this.vertices;this.rainbowColors?this.ctx.strokeStyle="hsl("+360*Math.abs(Math.sin(5*e))+", 100%, 50%)":this.ctx.strokeStyle=h.lerpColor(this.colorA,this.colorB,Math.abs(Math.sin(5*e))),this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2),this.ctx.beginPath();for(let t=0;t<=this.vertices;++t){var s=t*i,a=Math.cos(s)*this.noiseScale,r=Math.sin(s)*this.noiseScale,a=this.noise.simplex3(a,r,e),r=h.remap(a,-1,1,this.radiusMin,this.radiusMax),a=r*Math.cos(s),r=r*Math.sin(s);0===t?this.ctx.moveTo(a,r):this.ctx.lineTo(a,r)}this.ctx.stroke(),this.ctx.resetTransform()}resize(){this.radiusMin=Math.min(this.ctx.canvas.width,this.ctx.canvas.height)/2*this.radiusScaleMin,this.radiusMax=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/2*this.radiusScaleMax,this.radiusMin>this.radiusMax&&([this.radiusMin,this.radiusMax]=[this.radiusMax,this.radiusMin]),this.clear()}updateColors(t,e,i){super.updateColors(t,e,i),this.resize()}getSettings(){return[{prop:"vertices",type:"int",min:3,max:720,toCall:"resize"},{prop:"radiusScaleMin",type:"float",min:0,max:2,toCall:"resize"},{prop:"radiusScaleMax",type:"float",min:0,max:2,toCall:"resize"},{prop:"noiseScale",type:"float",min:0,max:2,toCall:"resize"},{prop:"fadingSpeed",type:"float",step:.001,min:0,max:.1},{prop:"rainbowColors",name:'<i class="fa-solid fa-rainbow"></i> rainbow colors',type:"bool"},this.getSeedSettings()]}}e.exports=a},{"../animation":2,"../utils":36}],7:[function(t,e,i){var s=t("../animation");let h=t("../utils");class a extends s{constructor(t,e,i,s,a=20,r=4){super(t,e,i,s,"Code writing animation","coding.js",`
Work in progress.
...
Coded with no external dependencies, using only canvas API.
`}update(t){++this.frame;for(let e=0;e<this.gridHeight;++e)for(let t=0;t<this.gridWidth;++t){var i=this.numAliveInMooreNeighborhood(t,e),s=this.getIdx(t,e);this.grid[s]<=0&&[3,6,7,8].includes(i)||1<=this.grid[s]&&[3,4,6,7,8].includes(i)?this.gridNext[s]=1:this.gridNext[s]=this.grid[s]-1}[this.grid,this.gridNext]=[this.gridNext,this.grid]}}e.exports=s},{"./game-of-life":10}],9:[function(t,e,i){var s=t("./game-of-life");let p=t("../utils");class a extends s{constructor(t,e,i,s,a=14,r=0,h=.4,n=!0,o=!0,l=!0,c=.75){super(t,e,i,s,a,r,h,l),this.name="isometric Conway's game of life",this.file="game-of-life-isometric.js",this.description=`
...
Coded with no external dependencies, using only canvas API.
`,this.fadeDeadCells=n,this.drawCellsGrid=o,this.gridSize=c,this.sqrt3=Math.sqrt(3),this.xShift=this.cellSize*this.sqrt3/2,this.yShift=this.cellSize/2,this.renderedGrid=null,this.renderedCubes=[];var d=p.createOffscreenCanvas(4*this.xShift,4*this.yShift).getContext("2d");this.drawIsoCube(d,0,3*this.yShift,!0,!0,this.colors,0,this.cellSize),this.renderedCubes.push(d.canvas);for(let t=1;t<this.cellSize;++t)d=p.createOffscreenCanvas(4*this.xShift,4*this.yShift).getContext("2d"),this.drawIsoCube(d,0,3*this.yShift,!0,!0,this.colorsAlt,-t,this.cellSize),this.renderedCubes.push(d.canvas)}drawIsoCube(t,e,i,s,a,r,h,n){var o=n*this.sqrt3/2,n=n/2;h*=-1,t.strokeStyle=r[0],t.fillStyle=r[3],t.beginPath(),p.pathClosedShape(t,[[e,i-2*n+h],[e+o,i-n+h],[e+2*o,i-2*n+h],[e+o,i-3*n+h]]),t.fill(),t.stroke(),s&&(t.fillStyle=r[2],t.beginPath(),p.pathClosedShape(t,[[e,i],[e+o,i+n],[e+o,i-n+h],[e,i-2*n+h]]),t.fill(),t.stroke()),a&&(t.fillStyle=r[1],t.beginPath(),p.pathClosedShape(t,[[e+o,i+n],[e+2*o,i],[e+2*o,i-2*n+h],[e+o,i-n+h]]),t.fill(),t.stroke())}drawCube(t,e,i,s=0,a=0){var r=t*this.xShift-e*this.xShift,h=(t+e+1)*this.yShift;this.drawIsoCube(this.ctx,r,h,!this.isAlive(t,e+1),!this.isAlive(t+1,e),i,s,this.cellSize-2*a)}drawGrid(s,t,e){var a=this.gridHeight*-this.xShift,r=this.gridHeight*this.yShift,h=this.gridWidth*this.xShift,n=this.gridWidth*this.yShift,i=(-this.gridHeight+this.gridWidth)*this.xShift,o=(this.gridHeight+this.gridWidth)*this.yShift,l=this.colors[0];for(let i=0;i<this.gridHeight;++i){let t=i*-this.xShift,e=i*this.yShift;p.drawLine(s,t,e,t+h,e+n,1,l),p.drawLine(s,-t,e,-t+a,e+r,1,l)}p.drawLine(s,0,0,h,n,3,l),p.drawLine(s,0,0,a,r,3,l),p.drawLine(s,a,r,i,o,3,l),p.drawLine(s,h,n,i,o,3,l)}drawPrerenderedCube(t,e,i){var s=t*this.xShift-e*this.xShift,t=(t+e+1)*this.yShift;this.ctx.drawImage(this.renderedCubes[i],s-+this.xShift,t-3*this.yShift)}mouseCellCord(t,e){}draw(){var t;this.clear(),this.drawCellsGrid&&(this.renderedGrid||((t=p.createOffscreenCanvas(this.ctx.canvas.width,this.ctx.canvas.height).getContext("2d")).translate(this.ctx.canvas.width/2,(1-this.gridSize)/2*this.ctx.canvas.height),this.drawGrid(t,0,0),this.renderedGrid=t.canvas),this.ctx.drawImage(this.renderedGrid,0,0)),this.ctx.translate(this.ctx.canvas.width/2,(1-this.gridSize)/2*this.ctx.canvas.height);for(let e=0;e<this.gridHeight;++e)for(let t=0;t<this.gridWidth;++t){var i=this.getVal(t,e);this.fadeDeadCells&&i>-(this.cellSize-2*this.cellBasePadding)?this.drawPrerenderedCube(t,e,Math.max(0,-i)):0<i&&this.drawPrerenderedCube(t,e,0)}this.ctx.resetTransform()}resize(){var t=Math.min(this.ctx.canvas.width,this.ctx.canvas.height),t=Math.ceil(this.gridSize*t/this.cellSize);this.resizeGrid(t,t),this.renderedGrid=null}getSettings(){return[{prop:"loopGrid",type:"bool"},{prop:"fadeDeadCells",type:"bool"},{prop:"drawCellsGrid",type:"bool"},{prop:"gridSize",type:"float",step:.01,min:0,max:2,toCall:"resize"},{prop:"spawnProb",type:"float",step:.01,min:0,max:1,toCall:"restart"},this.getSeedSettings()]}}e.exports=a},{"../utils":36,"./game-of-life":10}],10:[function(t,e,i){var s=t("../grid-animation");let d=t("../utils");class a extends s{constructor(t,e,i,s,a=12,r=1,h=.4,n=!0,o="random",l=5,c="size+color"){super(t,e,i,s,"Conway's game of life","game-of-life.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.cellSize=a,this.cellBasePadding=r,this.initialPaterns=["random","R Pentomino"],this.spawnProb=h,this.cellStyles=["square","circle"],this.cellStyle=this.assignIfRandom(o,d.randomChoice(this.cellStyles)),this.deadCellsFadingSteps=l,this.deadCellsFadingStyles=["color","size","size+color"],this.deadCellsFadingStyle=this.assignIfRandom(c,d.randomChoice(this.deadCellsFadingStyles)),this.loopGrid=n,this.mouseDown=!1,this.mouseVal=0}isAlive(t,e){return this.loopGrid?1<=this.getValWrap(t,e)?1:0:!(t<0||t>=this.gridWidth||e<0||e>=this.gridHeight)&&1<=this.getVal(t,e)?1:0}numAliveInMooreNeighborhood(t,e){return this.isAlive(t-1,e-1)+this.isAlive(t,e-1)+this.isAlive(t+1,e-1)+this.isAlive(t-1,e)+this.isAlive(t+1,e)+this.isAlive(t-1,e+1)+this.isAlive(t,e+1)+this.isAlive(t+1,e+1)}update(t){super.update(t);for(let e=0;e<this.gridHeight;++e)for(let t=0;t<this.gridWidth;++t){var i=this.numAliveInMooreNeighborhood(t,e),s=this.getIdx(t,e);2===i&&1<=this.grid[s]?this.gridNext[s]=this.grid[s]+1:this.gridNext[s]=3===i?Math.max(1,this.grid[s]+1):Math.min(0,this.grid[s]-1)}[this.grid,this.gridNext]=[this.gridNext,this.grid]}drawSquareCell(t,e,i){this.ctx.fillRect(t*this.cellSize+i,e*this.cellSize+i,this.cellSize-2*i,this.cellSize-2*i)}drawCircleCell(t,e,i){this.ctx.beginPath(),this.ctx.arc(t*this.cellSize+this.cellSize/2,e*this.cellSize+this.cellSize/2,this.cellSize/2-i,0,2*Math.PI,!1),this.ctx.fill()}draw(){this.clear(),this.ctx.translate(-(this.mapWidth*this.cellSize-this.ctx.canvas.width)/2,-(this.mapHeight*this.cellSize-this.ctx.canvas.height)/2),"square"===this.cellStyle?this.drawCell=this.drawSquareCell:this.drawCell=this.drawCircleCell;var r=(this.cellSize/2-this.cellBasePadding)/(this.deadCellsFadingSteps+1),h=this.deadCellsFadingStyle.includes("size"),n=this.deadCellsFadingStyle.includes("color");for(let a=0;a<this.gridHeight;++a)for(let t=0;t<this.gridWidth;++t){var o=this.getVal(t,a);let e=this.cellBasePadding,i=null,s=-1;if(0<o)i=this.colors[0];else for(let t=0;t<this.deadCellsFadingSteps;++t){if(o>s){i=n?this.colors[Math.min(t,this.colors.length-1)]:this.colors[0],h&&(e+=t*r);break}s*=2}i&&(this.ctx.fillStyle=i,this.drawCell(t,a,e))}this.ctx.resetTransform()}newCellState(t,e){return this.rand()<this.spawnProb?1:-99999}mouseAction(t,e){"down"===e?this.mouseDown=!0:"up"===e&&(this.mouseDown=!1),("down"===e||"move"===e&&this.mouseDown)&&(t=this.mouseCellCord(t.x,t.y),"down"===e&&(1===this.grid[t]?this.mouseVal=-99999:this.mouseVal=1),this.grid[t]!==this.mouseVal)&&(this.grid[t]=this.mouseVal,this.draw())}mouseCellCord(t,e){return Math.floor(t/this.cellSize)+Math.floor(e/this.cellSize)*this.gridWidth}getSettings(){return[{prop:"changeGrid",type:"text",value:"<click/touch>"},{prop:"loopGrid",type:"bool"},{prop:"cellSize",type:"int",min:4,max:32,toCall:"restart"},{prop:"cellStyle",type:"select",values:this.cellStyles},{prop:"deadCellsFadingSteps",type:"int",min:0,max:8},{prop:"deadCellsFadingStyle",type:"select",values:this.deadCellsFadingStyles},{prop:"spawnProb",type:"float",step:.01,min:0,max:1,toCall:"restart"},this.getSeedSettings()]}}e.exports=a},{"../grid-animation":32,"../utils":36}],11:[function(t,e,i){var s=t("../grid-animation");let n=t("../utils");class a extends s{constructor(t,e,i,s,a=7,r="random",h=.0051){super(t,e,i,s,"Glitch animation","glitch.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.funcNames=["with saddle point","BEALE","Rosenbrock","Styblinski-Tang"],this.functionToOptimize=this.assignIfRandom(a,y.randomChoice(this.funcNames)),this.funcClasses=[f,x,w,v],this.scale=r,this.rounding=h,this.autoRestart=n,this.autoRestartSteps=o,this.func=null,this.start=null,this.optims=null,this.imageData=null,this.drawScale=0,this.functionImageData=null,this.sgd=new l,this.momentum=new c,this.adagrad=new d,this.rmsprop=new p,this.adam=new m,this.adamax=new g,this.amsgrad=new u,this.optims=[this.sgd,this.momentum,this.adagrad,this.rmsprop,this.adam,this.adamax,this.amsgrad]}draw(){this.imageData&&this.ctx.putImageData(this.imageData,0,0),this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2);for(let t=0;t<this.optims.length;++t){var e,i,s,a=this.optims[t];[e,i]=a.w,a.update(this.func.grad(a.w)),[a,s]=a.w,isFinite(e)&&isFinite(i)&&isFinite(a)&&isFinite(s)&&(isNaN(e)||isNaN(i)||isNaN(a)||isNaN(s)||1e4<Math.abs(e)||1e4<Math.abs(i)||1e4<Math.abs(a)||1e4<Math.abs(s)||y.drawLine(this.ctx,e*this.drawScale,-i*this.drawScale,a*this.drawScale,-s*this.drawScale,2,this.colorsAlt[t]))}this.ctx.resetTransform(),this.imageData=this.ctx.getImageData(0,0,this.ctx.canvas.width,this.ctx.canvas.height),this.resetFont();let r=this.textYOffset+2*this.lineHeight;y.fillAndStrokeText(this.ctx,"Steps: "+this.frame,this.textXOffset,r),r+=this.lineHeight,y.fillAndStrokeText(this.ctx,"Optimizers:",this.textXOffset,r);for(let t=0;t<this.optims.length;++t){r+=this.lineHeight,this.ctx.fillStyle=this.colorsAlt[t];var h=this.optims[t].w[0].toFixed(this.rounding),n=this.optims[t].w[1].toFixed(this.rounding),h=this.optims[t].getName()+`: f([${h}, ${n}]) = `+this.func.val([h,n]).toFixed(this.rounding);y.fillAndStrokeText(this.ctx,h,this.textXOffset+16,r),y.fillCircle(this.ctx,this.textXOffset+3,r-4,3,this.colorsAlt[t])}this.autoRestart&&this.frame>=this.autoRestartSteps&&(this.start=null,this.resize())}drawFunction(){if(this.functionImageData)this.ctx.putImageData(this.functionImageData,0,0);else{this.clear();var h=this.ctx.canvas.width,n=this.ctx.canvas.height,o=h/2,l=n/2,t=this.func.getScale(),c=t/this.scale,d=t/.5,p=(this.drawScale=Math.min(h,n)/t/2*this.scale,this.ctx.fillStyle=this.colors[0],this.ctx.strokeStyle=this.bgColor,this.ctx.font="12px sans-serif",new Array(h*n));let i,s,a,r=0;a=this.func.hasGlobalMin()?(r=this.func.val(this.func.getGlobalMin()),i=[0,.125],s=1.5,0):(r=0,t=[this.func.val([0,0]),this.func.val([d,0]),this.func.val([0,d]),this.func.val([-d,0]),this.func.val([0,-d]),this.func.val([d,d]),this.func.val([-d,-d]),this.func.val([d,-d]),this.func.val([-d,d])],d=Math.min(...t),t=Math.max(...t),i=[d],s=1,(t-d)/29);for(let e=0;e<h;++e)for(let t=0;t<n;++t){for(var m=(e-o)/this.drawScale,g=-(t-l)/this.drawScale,u=this.func.val([m,g]),f=e+t*h;u>r+i[i.length-1];)i.push(i[i.length-1]*s+a);for(let t=1;t<i.length;++t)if(u<r+i[t]){p[f]=t-1;break}}var x=[];for(let t=0;t<i.length;++t)x.push(y.lerpColor(this.colorA,this.colorB,(t+1)/(i.length+1)));for(let e=0;e<h;++e)for(let t=0;t<n;++t){var v=e+t*h,w=-3*p[v]+p[v+1]+p[v+h]+p[v+1+h];this.ctx.fillStyle=x[p[v]],0!==w&&4!==w&&this.ctx.fillRect(e,t,1,1)}this.ctx.fillStyle=this.colors[0];let e=.5;6<c?e=2:3<c&&(e=1);for(let t=0;t<o/this.drawScale;t+=e)y.fillAndStrokeText(this.ctx,t.toFixed(1),o+t*this.drawScale,n-22),0!==t&&y.fillAndStrokeText(this.ctx,(-t).toFixed(1),o-t*this.drawScale,n-22);for(let t=0;t<l/this.drawScale;t+=e)y.fillAndStrokeText(this.ctx,(-t).toFixed(1),10,l+t*this.drawScale),0!==t&&y.fillAndStrokeText(this.ctx,t.toFixed(1),10,l-t*this.drawScale);this.functionImageData=this.ctx.getImageData(0,0,h,n),this.functionImageData.drawScale=this.drawScale,this.functionImageData.scale=this.scale}}drawLegend(){var t,e=this.ctx.canvas.width,i=this.ctx.canvas.height,s=e/2,a=i/2;this.textYOffset=22,this.textXOffset=50,this.resetFont(),y.fillAndStrokeText(this.ctx,this.func.getName(),this.textXOffset,this.textYOffset),this.func.hasGlobalMin()&&(this.textYOffset+=this.lineHeight,t=this.func.getGlobalMin(),y.fillAndStrokeText(this.ctx,`Optimum: f(x*) = ${this.func.val(t).toFixed(this.rounding)}, at x* = [${t[0]}, ${t[1]}]`,this.textXOffset,this.textYOffset,2),y.fillCircle(this.ctx,s+t[0]*this.drawScale,a+-t[1]*this.drawScale,2,this.colors[0])),this.textYOffset+=this.lineHeight,y.fillAndStrokeText(this.ctx,`Starting point: x0 = [${this.start[0].toFixed(this.rounding)}, ${this.start[1].toFixed(this.rounding)}], f(x0) = `+this.func.val(this.start).toFixed(this.rounding),this.textXOffset,this.textYOffset),this.imageData=this.ctx.getImageData(0,0,e,i)}mouseAction(t,e){"click"===e&&(this.start=[(t.x-this.ctx.canvas.width/2)/this.drawScale,-(t.y-this.ctx.canvas.height/2)/this.drawScale],this.resize())}resize(){this.frame=0;var t,e=new this.funcClasses[this.funcNames.indexOf(this.functionToOptimize)];null!==this.func&&this.func.getName()===e.getName()||(this.func=e,this.functionImageData=null,this.start=null),null===this.functionImageData||this.functionImageData.width===this.ctx.canvas.width&&this.functionImageData.height===this.ctx.canvas.height&&this.functionImageData.scale===this.scale||(this.functionImageData=null),null===this.start&&(this.start=this.func.getStartPoint()),this.drawFunction(),this.drawLegend();for(t of this.optims)t.init(this.start)}updateColors(t,e,i){super.updateColors(t,e,i),this.functionImageData=null,this.resize()}getSettings(){return[{prop:"functionToOptimize",type:"select",values:this.funcNames,toCall:"resize"},{prop:"selectStartingPoint",type:"text",value:"<click/touch>"},{prop:"scale",type:"float",step:.1,min:.5,max:1.5,toCall:"resize"},{prop:"autoRestart",type:"bool"},{prop:"autoRestartSteps",type:"int",step:1,min:100,max:1e4},{type:"separator"},{prop:"sgd.eta",type:"float",step:1e-5,min:0,max:.1},{prop:"momentum.eta",type:"float",step:1e-5,min:0,max:.1},{prop:"momentum.beta",type:"float",step:1e-4,min:0,max:1},{prop:"adagrad.eta",type:"float",step:1e-5,min:0,max:1},{prop:"rmsprop.eta",type:"float",step:1e-5,min:0,max:1},{prop:"adam.eta",type:"float",step:1e-5,min:0,max:1},{prop:"adam.beta1",type:"float",step:1e-5,min:0,max:1},{prop:"adam.beta2",type:"float",step:1e-5,min:0,max:1},{prop:"adamax.alpha",type:"float",step:1e-5,min:0,max:1},{prop:"adamax.beta1",type:"float",step:1e-5,min:0,max:1},{prop:"adamax.beta2",type:"float",step:1e-5,min:0,max:1},{prop:"amsgrad.alpha",type:"float",step:1e-5,min:0,max:1},{prop:"amsgrad.beta1",type:"float",step:1e-5,min:0,max:1},{prop:"amsgrad.beta2",type:"float",step:1e-5,min:0,max:1}]}}e.exports=h},{"../animation":2,"../utils":36}],13:[function(t,e,i){var s=t("../animation");let a=t("../utils");class r extends s{constructor(t,e,i,s,a=20,r=.6,h=.01,n=!1){super(t,e,i,s,"Matrix digital rain","matrix.js",`
...
`),this.particlesDensity=a,this.fillTriangles=r,this.drawParticles=h,this.distanceThreshold=n,this.width=0,this.height=0,this.particles=[],this.mouseDown=!1,this.mouseParticle={x:0,y:0,color:null}}drawTriangle(t,e,i){var s=Math.max(h.distVec2d(t,e),h.distVec2d(t,e),h.distVec2d(e,i));s>this.distanceThreshold||(this.ctx.beginPath(),h.pathClosedShape(this.ctx,[t,e,i]),e=h.lerpColor(t.color,this.bgColor,h.easeInSine(s/this.distanceThreshold)),this.fillTriangles?(this.ctx.fillStyle=e,this.ctx.fill()):(this.ctx.lineWidth=1,this.ctx.strokeStyle=e,this.ctx.stroke()))}update(t){super.update(t);for(var e of this.particles)e.x+=e.velX*t*this.speed,e.y+=e.velY*t*this.speed,(e.x<0||e.x>this.width)&&(e.velX*=-1),(e.y<0||e.y>this.height)&&(e.velY*=-1)}draw(){if(this.clear(),this.mouseDown&&this.particles.push(this.mouseParticle),0<this.particles.length){var e=r.triangulate(this.particles.map(function(t){return[t.x,t.y]}));for(let t=0;t<e.length;t+=3){var i=this.particles[e[t]],s=this.particles[e[t+1]],a=this.particles[e[t+2]];this.drawTriangle(i,s,a)}}if(this.drawParticles)for(var t of this.particles)h.fillCircle(this.ctx,t.x,t.y,2,t.color);this.mouseDown&&this.particles.pop()}spawnParticles(e,i,s,a){var r=s*a*this.particlesDensity;for(let t=0;t<r;t++)this.particles.push({x:this.rand()*s+e,y:this.rand()*a+i,velY:(2*this.rand()-1)/30,velX:(2*this.rand()-1)/30,color:h.randomChoice(this.colors,this.rand)})}restart(){this.particles=[],this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height,this.spawnParticles(0,0,this.width,this.height)}resize(){var t=this.ctx.canvas.width-this.width,e=this.ctx.canvas.height-this.height;0<t&&this.spawnParticles(this.width,0,t,this.height),0<e&&this.spawnParticles(0,this.height,this.width,e),(0<t||0<e)&&this.spawnParticles(this.width,this.height,t,e),this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height;let i=this.width,s=this.height;this.particles=this.particles.filter(function(t){return!(t.x<0||t.x>i||t.y<0||t.y>s)})}mouseAction(t,e){"down"===e?(this.mouseDown=!0,this.mouseParticle.color=h.randomChoice(this.colors,this.rand)):"up"===e?this.mouseDown=!1:("down"===e||"move"===e&&this.mouseDown)&&(this.mouseParticle.x=t.x,this.mouseParticle.y=t.y)}getSettings(){return[{prop:"particlesDensity",type:"float",step:1e-4,min:1e-4,max:.002,toCall:"restart"},{prop:"fillTriangles",type:"bool"},{prop:"drawParticles",type:"bool"},{prop:"distanceThreshold",type:"int",min:0,max:200},{prop:"speed",type:"float",step:.1,min:-4,max:4},{prop:"addAParticle",type:"text",value:"<hold mouse button/touch>"},this.getSeedSettings()]}}e.exports=a},{"../animation":2,"../delaunay":31,"../utils":36}],16:[function(t,e,i){var s=t("../animation");let l=t("../utils");class a extends s{constructor(t,e,i,s,a=100,r=.07,h=.05,n=.5,o=1){super(t,e,i,s,"noisy lines","noisy-lines.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.noiseXIncr=r,this.noiseYIncr=h,this.noiseRange=n,this.lines=a,this.speed=o,this.margin=.1}draw(){this.clear(),this.ctx.lineWidth=1,this.ctx.strokeStyle=this.colors[0];var t=this.ctx.canvas.width/2,e=this.ctx.canvas.height/2,i=t*(1-this.margin),s=e*(1-this.margin),a=2*s/(this.lines-1);this.ctx.translate(t,e);let r=.3*this.time;for(let t=0;t<this.lines;++t){var h=-s+t*a;let e=.3*this.time;r+=this.noiseYIncr*a/5,this.ctx.beginPath();for(let t=-i;t<=i;t+=5){e+=this.noiseXIncr;var n=l.remap(this.noise.perlin2(e,r),-1,1,1-this.noiseRange,1),o=t*n,n=h*n;t==-i?this.ctx.moveTo(o,n):this.ctx.lineTo(o,n)}this.ctx.stroke()}this.ctx.resetTransform()}getSettings(){return[{prop:"lines",type:"int",min:2,max:250},{prop:"noiseXIncr",type:"float",min:-.2,max:.2,step:.001},{prop:"noiseYIncr",type:"float",min:-.2,max:.2,step:.001},{prop:"noiseRange",type:"float",min:0,max:1,step:.01},{prop:"speed",type:"float",min:-8,max:8},this.getSeedSettings()]}}e.exports=a},{"../animation":2,"../utils":36}],17:[function(t,e,i){var s=t("../animation");let g=t("../utils");class a extends s{constructor(t,e,i,s,a=1e4,r="random",h=.03,n="random",o="random",l="random",c="random",d=!1,p=1,m=!1){super(t,e,i,s,"system of particles and attractors","particles-and-attractors.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.particles=[],this.numParticles=a,this.particlesSpeed=this.assignIfRandom(r,g.round(g.randomRange(.25,.5))),this.fadingSpeed=h,this.nextFadeStep=0,this.attractors=[],this.drawAttractors=d,this.numAttractors=this.assignIfRandom(n,g.randomInt(3,7)),this.centralAttractor=this.assignIfRandom(o,g.randomChoice([!1,!0])),this.attractorsSystems=["orbits","eights","circle"],this.attractorsSystem=this.assignIfRandom(l,g.randomChoice(this.attractorsSystems)),this.attractorsSpeed=this.assignIfRandom(c,g.round(g.randomRange(.05,.1)*g.randomChoice([-1,1]))),this.attractorsPosition=0,this.startingPosition=g.randomRange(0,10),this.directionScale=1,this.scale=p,this.rainbowColors=m,this.mouseDown=!1,this.mouseAttractor={x:0,y:0},this.setup()}setup(){this.particles=[];for(let t=0;t<this.numParticles;++t)this.particles.push(g.rotateVec2d({x:g.randomRange(1,100,this.rand),y:0,prevX:0,prevY:0},t))}update(e){super.update(e),this.nextFadeStep=this.fadingSpeed*e/33,this.attractorsPosition+=e/1e3*this.attractorsSpeed;var i,s=this.startingPosition+this.attractorsPosition,a=this.numAttractors+1-this.centralAttractor,r=1-this.centralAttractor;if(this.attractors=[],"orbits"===this.attractorsSystem){var h=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/(2*(this.numAttractors-1));for(let t=r;t<a;++t)this.attractors.push(g.rotateVec2d(g.createVec2d(t*h,0),s*t))}else if("eights"===this.attractorsSystem){var n=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/this.numAttractors;for(let t=r;t<a;++t)this.attractors.push(g.rotateVec2d(g.createVec2d(t*Math.sin(s*Math.PI/2)*n,0),s*t))}else if("circle"===this.attractorsSystem){this.centralAttractor&&this.attractors.push({x:0,y:0});var o=.4*Math.min(this.ctx.canvas.width,this.ctx.canvas.height);for(let t=1;t<a;++t)this.attractors.push(g.rotateVec2d(g.createVec2d(o,0),2*(s+t)*Math.PI/(a-1)))}this.mouseDown&&this.attractors.push(this.mouseAttractor);for(i of this.particles){let t=0;for(var l of this.attractors)t+=Math.atan2(l.y-i.y,l.x-i.x)*this.directionScale;i.prevX=i.x,i.prevY=i.y,i.x+=Math.cos(t)*this.particlesSpeed*e,i.y+=Math.sin(t)*this.particlesSpeed*e}}draw(){this.fadeOut(this.nextFadeStep),this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2),this.ctx.scale(this.scale,this.scale);var t,e=this.rainbowColors?`hsl(${this.time/5*360}, 100%, 75%)`:this.colors[0];for(t of this.particles)g.drawLine(this.ctx,t.prevX,t.prevY,t.x,t.y,1,e);if(this.drawAttractors)for(var i of this.attractors)g.fillCircle(this.ctx,i.x,i.y,5,this.colorsAlt[2]);this.ctx.resetTransform()}resize(){this.clear()}restart(){super.restart(),this.attractorsPosition=0,this.setup()}mouseAction(t,e){"down"===e?this.mouseDown=!0:"up"===e?this.mouseDown=!1:("down"===e||"move"===e&&this.mouseDown)&&(console.log("attractor!"),this.mouseAttractor={x:(t.x-this.ctx.canvas.width/2)/this.scale,y:(t.y-this.ctx.canvas.height/2)/this.scale})}updateColors(t,e,i){super.updateColors(t,e,i),this.resize()}getSettings(){return[{prop:"numParticles",type:"int",min:1e3,max:2e4,toCall:"setup"},{prop:"particlesSpeed",type:"float",min:.1,max:1.5},{prop:"fadingSpeed",type:"float",step:.001,min:0,max:.1},{prop:"attractorsSystem",type:"select",values:this.attractorsSystems},{prop:"numAttractors",type:"int",min:3,max:7},{prop:"centralAttractor",type:"bool"},{prop:"attractorsSpeed",type:"float",min:-.25,max:.25},{prop:"directionScale",type:"float",min:0,max:4},{prop:"addAnAttractor",type:"text",value:"<hold mouse button/touch>"},{prop:"drawAttractors",type:"bool"},{prop:"scale",type:"float",min:.05,max:1.95},{prop:"rainbowColors",name:'<i class="fa-solid fa-rainbow"></i> rainbow colors',type:"bool"}]}}e.exports=a},{"../animation":2,"../utils":36}],18:[function(t,e,i){var s=t("../animation");let d=t("../utils");class a extends s{constructor(t,e,i,s,a=1500,r="random",h="random",n="random",o="random",l="random",c=1){super(t,e,i,s,"vortex of particles","particles-vortex.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.pointsDensity=r,this.drawPoints=h,this.maxPointsInNode=a,this.noiseThreshold=l,this.drawLeafNodes=c,this.noiseScale=n,this.noiseSpeed=o,this.noiseSpeed.x=this.assignIfRandom(this.noiseSpeed.x,d.round(d.randomRange(-1,1),1)),this.noiseSpeed.y=this.assignIfRandom(this.noiseSpeed.y,d.round(d.randomRange(-1,1),1)),this.minNodeSize=4,this.width=0,this.height=0,this.noisePos={x:0,y:0,z:0}}update(t){this.noisePos.x+=this.noiseSpeed.x*t/1e3*10,this.noisePos.y+=this.noiseSpeed.y*t/1e3*10,this.noisePos.z+=this.noiseSpeed.z*t/1e3*.05,super.update(t)}generatePoints(){var i=[],s=1/this.pointsDensity,a=s/2,r=d.Mulberry32(this.seed);for(let e=a;e<this.width;e+=s)for(let t=a;t<this.height;t+=s){var h=this.noise.perlin3((e+this.noisePos.x)*this.noiseScale,(t+this.noisePos.y)*this.noiseScale,this.noisePos.z);Math.abs(h)<=this.noiseThreshold&&i.push({x:e-a+r()*s,y:t-a+r()*s})}return i}quadTree(t,e,i,s,a){var r=i/2,h=i/4;if(s.length<=this.maxPointsInNode||i<=this.minNodeSize)(i>this.minNodeSize||this.drawLeafNodes)&&this.ctx.strokeRect(t-r,e-r,i,i);else{var n,o=[],l=[],c=[],d=[];for(n of s)n.x<t&&n.y>=e?o.push(n):n.x>=t&&n.y>=e?l.push(n):n.x>=t&&n.y<e?d.push(n):n.x<t&&n.y<e&&c.push(n);this.quadTree(t+h,e+h,r,l,++a),this.quadTree(t+h,e-h,r,d,a),this.quadTree(t-h,e+h,r,o,a),this.quadTree(t-h,e-h,r,c,a)}}draw(){this.clear(),this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height;var t=Math.max(this.width,this.height),e=this.generatePoints();if(this.ctx.lineWidth=1,this.ctx.strokeStyle=this.colors[0],this.quadTree(this.width/2,this.height/2,t,e,0),this.drawPoints)if(this.ctx.fillStyle=this.colorsAlt[1],this.pointsDensity<.5)for(var i of e)d.fillCircle(this.ctx,i.x,i.y,2,this.colorsAlt[1]);else for(var s of e)this.ctx.fillRect(s.x,s.y,2,2)}restart(){this.noisePos={x:0,y:0,z:0},super.restart()}getSettings(){return[{prop:"maxPointsInNode",type:"int",min:1,max:16},{prop:"pointsDensity",type:"float",step:.1,min:.1,max:.7},{prop:"noiseScale",type:"float",step:1e-4,min:5e-4,max:.0125},{prop:"noiseSpeed.x",type:"float",step:.1,min:-10,max:10},{prop:"noiseSpeed.y",type:"float",step:.1,min:-10,max:10},{prop:"noiseSpeed.z",type:"float",step:.1,min:-10,max:10},{prop:"noiseThreshold",type:"float",min:0,max:.15,step:.001},{prop:"drawLeafNodes",type:"bool"},{prop:"drawPoints",type:"bool"},this.getSeedSettings()]}}e.exports=a},{"../animation":2,"../utils":36}],22:[function(t,e,i){var s=t("../animation");let a=t("../animation-queue"),d=t("../utils");class n{constructor(t,e){this.depth=null,this.rand=e,this.animQueue=new a,this.children=[],this.positions={0:{x:0,y:0,from:[1,2]},1:{x:1,y:0,from:[0,3]},2:{x:0,y:1,from:[0,3]},3:{x:1,y:1,from:[1,2]}},this.setDepth(t)}update(t){if(0!==this.children.length){for(var e of this.children)e.object.update(t);for(;0<this.animQueue.step(t);){var o,l,c=new Set;for(o of this.children)c.add(o.position);let t=new Set([0,1,2,3]),e=d.setsDifference(t,c).values().next().value,i=this.positions[e],s=d.randomChoice(i.from,this.rand),a=null;for(l of this.children)if(l.position==s){a=l;break}let r=this.positions[a.position],h=this.depth/2,n=(a.position=e,d.easeInOutSine);this.animQueue.push(function(t){var e=Math.min(t,h)/h;return a.x=d.lerp(r.x,i.x,n(e)),a.y=d.lerp(r.y,i.y,n(e)),t-h})}}}setDepth(t){if(0===t&&this.children.length)this.children=[],this.animQueue.clear();else if(0<t)if(0===this.children.length){this.children=[{object:new n(t-1,this.rand),position:0},{object:new n(t-1,this.rand),position:1},{object:new n(t-1,this.rand),position:2}];for(var e of this.children)e.x=this.positions[e.position].x,e.y=this.positions[e.position].y}else for(var i of this.children)i.object.setDepth(t-1);this.depth=t}draw(t,e){var i=e/2;if(0===this.children.length)t.strokeRect(-.5*e,-.5*e,i,i),t.strokeRect(0,-.5*e,i,i),t.strokeRect(-.5*e,0,i,i),t.strokeRect(0,0,i,i);else{t.strokeRect(-.5*e,-.5*e,e,e);for(var s of this.children)t.save(),t.translate((-.5+s.x)*i,(-.5+s.y)*i),s.object.draw(t,i),t.restore()}}}class r extends s{constructor(t,e,i,s,a=7,r=1,h=!1){super(t,e,i,s,"recursive rectangles","recursion-rectangles.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.depth=a,this.speed=r,this.contain=h,this.object=new n(this.depth,this.rand)}update(t){t=t/1e3*this.speed,this.object.update(t)}draw(){this.clear();let t=0;this.contain?(t=Math.min(this.ctx.canvas.width,this.ctx.canvas.height),this.ctx.translate((this.ctx.canvas.width-t)/2,(this.ctx.canvas.height-t)/2)):t=Math.max(this.ctx.canvas.width,this.ctx.canvas.height),this.ctx.lineWidth=1,this.ctx.strokeStyle=this.colors[0],this.ctx.fillStyle=this.colors[0],this.ctx.translate(t/2,t/2),this.object.draw(this.ctx,t),this.ctx.resetTransform()}updateDepth(){this.object.setDepth(this.depth)}getSettings(){return[{prop:"depth",type:"int",min:2,max:9,toCall:"updateDepth"},{prop:"speed",type:"float",step:.25,min:.5,max:8},{prop:"contain",type:"bool"},this.getSeedSettings()]}}e.exports=r},{"../animation":2,"../animation-queue":1,"../utils":36}],23:[function(t,e,i){var s=t("../grid-animation");let a=t("../utils");class r extends s{constructor(t,e,i,s,a=9,r=3,h=3){super(t,e,i,s,"rock-paper-scissors automata","rock-paper-scissors-automata.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.numElements=r,this.elementPadding=h,this.cmpDuration=n,this.swapDuration=o,this.speed=l,this.showStats=c,this.sortAlgoNames=["selection sort","bubble sort","insertion sort","quick sort","merge sort","heap sort","gnome sort","shaker sort"],this.sortAlgoClasses=[m,d,g,f,u,x,v,w],this.sortingAlgorithm=this.assignIfRandom(a,p.randomChoice(this.sortAlgoNames)),this.initialOrderTypes=["random","sorted","reverse sorted","evens then odds","nearly sorted","few unique"],this.initialOrder="random",this.cmpTotal=0,this.cmpCount=0,this.accessTotal=0,this.accessCount=0,this.setup()}setup(){let i=this.numElements,e=(this.animQueue=new o,null);if(e="few unique"===this.initialOrder?Array.from({length:i},(t,e)=>(e%10+1)/10*i):Array.from({length:i},(t,e)=>e+1),["random","few unique"].includes(this.initialOrder)?p.randomShuffle(e,this.rand):"reverse sorted"===this.initialOrder?e=e.reverse():"evens then odds"===this.initialOrder&&(e=e.sort((t,e)=>t%2+t/(i+1)-(e%2+e/(i+1)))),"nearly sorted"===this.initialOrder){var s=Math.floor(i/5);for(let t=0;t<s;++t){var a=p.randomInt(0,i,this.rand),r=p.randomInt(0,i,this.rand);[e[a],e[r]]=[e[r],e[a]]}}this.elements=[];for(let t=0;t<i;++t){var h=e[t]/i,n=p.lerpColor(this.colors[0],this.colors[2],h);this.elements.push({val:h,pos:t,color:n,z:0})}var t=new this.sortAlgoClasses[this.sortAlgoNames.indexOf(this.sortingAlgorithm)](this.elements);this.moves=t.getMoves(),this.name=t.getName()+" algorithm visualization",this.timeComplexity=t.getTimeComplexity(),this.spaceComplexity=t.getSpaceComplexity(),this.cmpTotal=t.cmpCount,this.cmpCount=0,this.accessTotal=t.readCount+t.writeCount,this.accessCount=0}update(t){for(t=t/1e3*this.speed,this.time+=t,++this.frame;0<this.animQueue.step(t);){if(!this.moves.length)return;var d=this.moves[0];let l=t=>t<.5?p.easeInOutCubic(2*t):1-p.easeInOutCubic(2*t-1),c=p.easeInOutSine;if("cmp"===d[0]){let i=d[1],s=d[2],a=i.color,r=s.color,h=this.colorsAlt[3],n=this.cmpDuration;++this.cmpCount,this.accessCount+=2,this.animQueue.push(function(t){var e=Math.min(t,n)/n;return i.color=p.lerpColor(a,h,l(e)),s.color=p.lerpColor(r,h,l(e)),t-n})}else if("swap"===d[0]){let i=d[1],e=d[2],s=[],a=[],r=[],h=this.colorsAlt[1],n=this.frame,o=this.swapDuration*i.length;2===i.length?this.accessCount+=2:this.accessCount+=2*i.length;for(let t=0;t<i.length;++t)s.push(i[t].pos),a.push(e[t].pos),r.push(i[t].color);this.animQueue.push(function(t){var e=Math.min(t,o)/o;for(let t=0;t<i.length;++t)i[t].z=n,i[t].color=p.lerpColor(r[t],h,l(e)),i[t].pos=p.lerp(s[t],a[t],c(e));return t-o})}this.moves.shift()}}draw(){this.clear();var t,e,i=this.ctx.canvas.height,s=this.ctx.canvas.width/this.numElements;this.elements=this.elements.sort((t,e)=>t.z-e.z);for(t of this.elements){var a=t.pos*s+this.elementPadding/2,r=t.val*i;this.ctx.fillStyle=t.color,this.ctx.fillRect(a,0,s-this.elementPadding,r)}this.showStats&&(this.resetFont(),e=["Sorting algorithm: "+this.sortingAlgorithm,"Theoretical time complexity: "+this.timeComplexity,"Theoretical space complexity: "+this.spaceComplexity,"Number of elements: "+this.numElements,`Number of elements comparisons: ${this.cmpCount} / `+this.cmpTotal,`Number of array accesses (read/write): ${this.accessCount} / `+this.accessTotal],this.drawTextLines(e,this.lineHeight,this.ctx.canvas.height-(e.length+1)*this.lineHeight))}restart(){super.restart(),this.setup()}getSettings(){return[{prop:"initialOrder",type:"select",values:this.initialOrderTypes,toCall:"restart"},{prop:"sortingAlgorithm",type:"select",values:this.sortAlgoNames,toCall:"restart"},{prop:"numElements",label:"number of elements (n)",type:"int",min:8,max:256,toCall:"restart"},{prop:"speed",type:"float",step:.25,min:.5,max:8},{prop:"showStats",type:"bool"},this.getSeedSettings()]}}e.exports=r},{"../animation":2,"../animation-queue":1,"../utils":36}],28:[function(t,e,i){var s=t("../animation");let d=t("../utils");class a extends s{constructor(t,e,i,s,a=500,r=0,h=!1,n=1,o=1,l="random",c=!1){super(t,e,i,s,"shapes dancing in a circle","spinning-shapes.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.shapeNames=["circles","points","lines","triangles","rectangles","pentagons","hexagons","heptagons","octagons"],this.vertices=this.assignIfRandom(r,d.randomInt(0,8)),this.updateName(),this.rotateShapes=h,this.shapes=a,this.distanceBase=.6,this.distanceRange=.2,this.sizeBase=.2,this.sizeRange=.12,this.scale=n,this.colorsScale=o,this.colorsShift=this.assignIfRandom(l,d.randomChoice([0,3.14])),this.rainbowColors=c}updateName(){this.name=this.shapeNames[this.vertices]+' "dancing" in a circle'}draw(){this.clear();var e=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/3*this.scale;this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2);for(let t=0;t<this.shapes;++t){var i=t/this.shapes*2*Math.PI,s=(this.distanceBase+this.distanceRange*Math.cos(6*i+Math.cos(8*i+this.time/2)))*e,a=Math.cos(i)*s,s=Math.sin(i)*s,r=9*i-this.time,h=(this.sizeBase+this.sizeRange*Math.cos(r))*e,r=(Math.cos((r+this.colorsShift)*this.colorsScale)+1)/2;this.rainbowColors?this.ctx.strokeStyle=`hsl(${360*r}, 100%, 75%)`:this.ctx.strokeStyle=d.lerpColor(this.colorA,this.colorB,r),this.ctx.lineWidth=1,this.ctx.beginPath(),0===this.vertices&&d.pathCircle(this.ctx,a,s,h),1===this.vertices?d.pathCircle(this.ctx,a,s,1):d.pathPolygon(this.ctx,a,s,h,this.vertices,i*this.rotateShapes),this.ctx.stroke()}this.ctx.resetTransform()}getSettings(){return[{prop:"vertices",type:"int",min:0,max:8,toCall:"updateName"},{prop:"shapes",type:"int",min:0,max:2500},{prop:"rotateShapes",type:"bool"},{prop:"distanceRange",type:"float",min:0,max:this.distanceBase},{prop:"sizeRange",type:"float",min:0,max:this.sizeBase},{prop:"scale",type:"float",min:.05,max:1.95},{prop:"speed",type:"float",step:.1,min:-4,max:4},{prop:"colorsShift",type:"float",min:0,max:3.14},{prop:"colorsScale",type:"float",min:.05,max:2},{prop:"rainbowColors",name:'<i class="fa-solid fa-rainbow"></i> rainbow colors',type:"bool"}]}}e.exports=a},{"../animation":2,"../utils":36}],29:[function(t,e,i){var s=t("../animation");let l=t("../utils");class a extends s{constructor(t,e,i,s,a=4e3,r=2,h="random",n=!0,o=1){super(t,e,i,s,"spirograph","spirograph.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.vertices=a,this.lineLength=r,this.maxGears=5,this.rescaleToFit=n,this.scale=o,this.speed=1,this.gearCount=this.assignIfRandom(h,l.randomInt(2,this.maxGears)),this.gearNames=["zero","one","two","three","four","five"],this.updateName(),this.setup()}setup(){this.gears=[];for(let t=0;t<this.maxGears;++t)this.gears.push({radius:l.round(l.randomRange(0,100),2),rate:l.round(l.randomRange(-100,100),2),phase:.005*t})}updateName(){this.name="spirograph with "+this.gearNames[this.gearCount]+" gears"}getXY(e,i,s=1){let a=0,r=0;for(let t=0;t<this.gearCount;++t){var h=this.gears[t];a+=h.radius*s*Math.cos(h.rate*(e+i*h.phase)),r+=h.radius*s*Math.sin(h.rate*(e+i*h.phase))}return{x:a,y:r}}draw(){this.clear();let i=1;if(this.rescaleToFit){let e=0;for(let t=0;t<this.gearCount;++t)e+=this.gears[t].radius;i=Math.min(this.ctx.canvas.width,this.ctx.canvas.height)/2/e}this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2),this.ctx.scale(this.scale,this.scale);var e=Math.PI*this.lineLength/this.vertices;let s=this.getXY(0,this.time,i);for(let t=1;t<this.vertices;++t){var a=this.getXY(t*e,this.time,i),r=l.lerpColor(this.colorA,this.colorB,t/this.vertices);l.drawLine(this.ctx,s.x,s.y,a.x,a.y,1,r),s=a}this.ctx.resetTransform()}getSettings(){let e=[{prop:"vertices",type:"int",min:100,max:32e3},{prop:"lineLength",type:"float",step:.25,min:1,max:16},{prop:"gearCount",type:"int",min:2,max:this.maxGears,toCall:"updateName"},{prop:"rescaleToFit",type:"bool"},{prop:"scale",type:"float",min:.25,max:4},{prop:"speed",type:"float",step:.1,min:-4,max:4},{type:"separator"}];for(let t=0;t<this.maxGears;++t)e=e.concat([{prop:`gears[${t}].radius`,type:"float",step:.01,min:0,max:100},{prop:`gears[${t}].rate`,type:"float",step:.01,min:-100,max:100},{prop:`gears[${t}].phase`,type:"float",step:.001,min:-.1,max:.1}]);return e}}e.exports=a},{"../animation":2,"../utils":36}],30:[function(t,e,i){var s=t("../animation");let o=t("../utils");class a extends s{constructor(t,e,i,s,a=64){super(t,e,i,s,"vectors","vectors.js",`
Vectors.

Coded with no external dependencies, using only canvas API.
`),this.distance=a,this.mouseRange=2*a,this.dashLenght=16,this.mousePoint=o.createVec2d(0,0)}draw(){this.clear(),this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2);var t,i=Math.floor(this.ctx.canvas.width/this.distance),s=Math.floor(this.ctx.canvas.height/this.distance),a=[],r=[];for(let e=0;e<i;++e)for(let t=0;t<s;++t){var h=(e-(i-1)/2)*this.distance,n=(t-(s-1)/2)*this.distance,h=o.createVec2d(h,n);a.push(h),o.distVec2d(this.mousePoint,h)<this.mouseRange&&r.push(h)}this.ctx.strokeStyle=this.colors[0],this.ctx.setLineDash([this.dashLenght]),this.ctx.lineDashOffset=-this.dashLenght*(this.time%30);for(t of a)for(var e of r)o.drawLine(this.ctx,t.x,t.y,e.x,e.y,.25);this.ctx.setLineDash([]),this.ctx.resetTransform()}mouseAction(t,e){"move"===e&&(this.mousePoint=o.createVec2d(t.x-this.ctx.canvas.width/2,t.y-this.ctx.canvas.height/2))}getSettings(){return[{prop:"distance",type:"int",min:32,max:256},{prop:"mouseRange",type:"int",min:32,max:512}]}}e.exports=a},{"../animation":2,"../utils":36}],31:[function(t,e,i){var S;function C(t,e,i,s){var a,r,h,n,o,l,c,d=t[e][0],p=t[e][1],m=t[i][0],g=t[i][1],u=t[s][0],t=t[s][1],f=Math.abs(p-g),x=Math.abs(g-t);if(f<S&&x<S)throw new Error("Eek! Coincident points!");return d=f<S?(h=-(u-m)/(t-g))*((a=(m+d)/2)-(o=(m+u)/2))+(c=(g+t)/2):x<S?(r=-(m-d)/(g-p))*((a=(u+m)/2)-(n=(d+m)/2))+(l=(p+g)/2):(a=((r=-(m-d)/(g-p))*(n=(d+m)/2)-(h=-(u-m)/(t-g))*(o=(m+u)/2)+(c=(g+t)/2)-(l=(p+g)/2))/(r-h),x<f?r*(a-n)+l:h*(a-o)+c),{i:e,j:i,k:s,x:a,y:d,r:(u=m-a)*u+(t=g-d)*t}}S=1/1048576,void 0!==e&&(e.exports={triangulate:function(s,t){var e,i,a,r,h,n,o,l,c,d,p,m=s.length;if(m<3)return[];if(s=s.slice(0),t)for(e=m;e--;)s[e]=s[e][t];for(a=new Array(m),e=m;e--;)a[e]=e;for(a.sort(function(t,e){var i=s[e][0]-s[t][0];return 0!=i?i:t-e}),r=function(t){for(var e,i,s,a=Number.POSITIVE_INFINITY,r=Number.POSITIVE_INFINITY,h=Number.NEGATIVE_INFINITY,n=Number.NEGATIVE_INFINITY,o=t.length;o--;)t[o][0]<a&&(a=t[o][0]),t[o][0]>h&&(h=t[o][0]),t[o][1]<r&&(r=t[o][1]),t[o][1]>n&&(n=t[o][1]);return s=n-r,[[(i=a+.5*(e=h-a))-20*(e=Math.max(e,s)),(s=r+.5*s)-e],[i,s+20*e],[i+20*e,s-e]]}(s),s.push(r[0],r[1],r[2]),h=[C(s,m+0,m+1,m+2)],n=[],o=[],e=a.length;e--;o.length=0){for(p=a[e],i=h.length;i--;)0<(l=s[p][0]-h[i].x)&&l*l>h[i].r?(n.push(h[i]),h.splice(i,1)):l*l+(l=s[p][1]-h[i].y)*l-h[i].r>S||(o.push(h[i].i,h[i].j,h[i].j,h[i].k,h[i].k,h[i].i),h.splice(i,1));v=x=f=u=y=g=w=void 0;for(var g,u,f,x,v,w=o,y=w.length;y;)for(f=w[--y],u=w[--y],g=y;g;)if(v=w[--g],u===(x=w[--g])&&f===v||u===v&&f===x){w.splice(y,2),w.splice(g,2);break}for(i=o.length;i;)d=o[--i],c=o[--i],h.push(C(s,c,d,p))}for(e=h.length;e--;)n.push(h[e]);for(h.length=0,e=n.length;e--;)n[e].i<m&&n[e].j<m&&n[e].k<m&&h.push(n[e].i,n[e].j,n[e].k);return h},contains:function(t,e){var i,s,a,r,h;return e[0]<t[0][0]&&e[0]<t[1][0]&&e[0]<t[2][0]||e[0]>t[0][0]&&e[0]>t[1][0]&&e[0]>t[2][0]||e[1]<t[0][1]&&e[1]<t[1][1]&&e[1]<t[2][1]||e[1]>t[0][1]&&e[1]>t[1][1]&&e[1]>t[2][1]||(i=t[1][0]-t[0][0],h=t[2][0]-t[0][0],s=t[1][1]-t[0][1],0==(a=i*(r=t[2][1]-t[0][1])-h*s))||(r=(r*(e[0]-t[0][0])-h*(e[1]-t[0][1]))/a,h=(i*(e[1]-t[0][1])-s*(e[0]-t[0][0]))/a,r<0)||h<0||1<r+h?null:[r,h]}})},{}],32:[function(t,e,i){t=t("./animation");class s extends t{constructor(t,e,i,s,a="",r="",h="",n="random"){super(t,e,i,s,a,r,h,n),this.gridWidth=0,this.gridHeight=0,this.grid=null,this.gridNext=null}getIdx(t,e){return t+e*this.gridWidth}getIdxWrap(t,e){return(t+this.gridWidth)%this.gridWidth+(e+this.gridHeight)%this.gridHeight*this.gridWidth}getVal(t,e){return this.grid[this.getIdx(t,e)]}getValWrap(t,e){return this.grid[this.getIdxWrap(t,e)]}newCellState(t,e){return 0}resize(){var t=Math.ceil(this.ctx.canvas.width/this.cellSize),e=Math.ceil(this.ctx.canvas.height/this.cellSize);this.resizeGrid(t,e)}resizeGrid(i,t){var s=new Array(i*t);for(let e=0;e<t;++e)for(let t=0;t<i;++t){var a=t+e*i;t<this.gridWidth&&e<this.gridHeight?s[a]=this.grid[this.getIdx(t,e)]:s[a]=this.newCellState(t,e)}delete this.grid,delete this.gridNext,this.grid=s,this.gridNext=[...this.grid],this.gridWidth=i,this.gridHeight=t}restart(){this.gridWidth=0,this.gridHeight=0,super.restart()}}e.exports=s},{"./animation":2}],33:[function(require,module,exports){let Utils=require("./utils"),ThreeNPlusOne=require("./animations/3n+1"),BriansBrainAutomata=require("./animations/brians-brain-automata"),Cardioids=require("./animations/cardioids"),CircularWaves=require("./animations/circular-waves"),Coding=require("./animations/coding"),DayAndNightAutomata=require("./animations/day-and-night-automata"),GameOfLife=require("./animations/game-of-life"),GameOfLifeIsometric=require("./animations/game-of-life-isometric"),Glitch=require("./animations/glitch"),GradientDescent=require("./animations/gradient-descent"),Matrix=require("./animations/matrix"),MLinPL=require("./animations/mlinpl"),Network=require("./animations/network"),NoisyLines=require("./animations/noisy-lines"),ParticlesAndAttractors=require("./animations/particles-and-attractors"),ParticlesVortex=require("./animations/particles-vortex"),ParticlesWaves=require("./animations/particles-waves"),PerlinNoiseParticles=require("./animations/perlin-noise-particles"),RockPaperScissorsAutomata=require("./animations/rock-paper-scissors-automata"),SandAutomata=require("./animations/sand-automata"),Quadtree=require("./animations/quadtree"),RecursiveRectangles=require("./animations/recursive-rectangles"),ShortestPath=require("./animations/shortest-path"),SineWaves=require("./animations/sine-waves"),Sorting=require("./animations/sorting"),SpinningShapes=require("./animations/spinning-shapes"),Spirograph=require("./animations/spirograph"),Vectors=require("./animations/vectors"),canvas=document.getElementById("background"),container=document.getElementById("background-container"),fps=30,framesInterval=1e3/fps,then=0,paused=!1,width=0,height=0,lastWidth=0,lastHeight=0,resolution="fit",fixedWidth=0,fixedHeight=0,sampleSize=30,frames=0,avgDrawTime=0,avgElapsedTime=0,trueThen=0,bgColors={black:"#000000",white:"#FFFFFF"},bgColor="#FFFFFF",colors=["#349BA9","#41B8AD","#73D4AD","#AEEABF","#73D4AD","#41B8AD"],colorsAlt=["#602180","#B6245C","#E14F3B","#EC8C4D","#FFF202","#99F32B","#106AA6","#283B93"],content=document.getElementById("me"),elemBgShow=document.getElementById("background-show"),elemBgName=document.getElementById("background-name"),elemBgDesc=document.getElementById("background-description"),elemBgPrev=document.getElementById("background-previous"),elemBgNext=document.getElementById("background-next"),elemBgCode=document.getElementById("background-code"),elemBgReset=document.getElementById("background-reset"),elemBgRestart=document.getElementById("background-restart"),elemBgPlayPause=document.getElementById("background-play-pause"),elemBgSettings=document.getElementById("background-settings"),elemBgSettingsControls=document.getElementById("background-settings-controls"),elemBgSettingsClose=document.getElementById("background-settings-close"),elemBgStats=document.getElementById("background-stats"),elemBgAnimationSelect=document.getElementById("background-settings-animation-select"),elemBgAnimationFps=document.getElementById("background-settings-animation-fps"),elemBgAnimationResolution=document.getElementById("background-settings-animation-resolution"),elemBgColor=document.getElementById("background-settings-bg-color");if(canvas){let animations=[{class:ThreeNPlusOne,name:"3n+1"},{class:BriansBrainAutomata,name:"brian's brain automata"},{class:Cardioids,name:"cardioids"},{class:CircularWaves,name:"circular waves"},{class:DayAndNightAutomata,name:"day and night automata"},{class:GameOfLife,name:"game of life"},{class:GameOfLifeIsometric,name:"isometric game of life"},{class:Glitch,name:"glitch",startAnimation:!1},{class:GradientDescent,name:"gradient descent"},{class:Matrix,name:"matrix rain"},{class:MLinPL,name:"ml in pl"},{class:Network,name:"network"},{class:NoisyLines,name:"noisy lines"},{class:ParticlesAndAttractors,name:"particles and attractors"},{class:ParticlesVortex,name:"particles vortex"},{class:ParticlesWaves,name:"particles waves"},{class:PerlinNoiseParticles,name:"perlin noise"},{class:RockPaperScissorsAutomata,name:"rock-paper-scissors automata"},{class:SandAutomata,name:"sand automata"},{class:Quadtree,name:"quadtree",startAnimation:!1},{class:RecursiveRectangles,name:"recursive rectangles",startAnimation:!1},{class:SineWaves,name:"sine waves"},{class:ShortestPath,name:"shortest path"},{class:Sorting,name:"sorting"},{class:SpinningShapes,name:"spinning shapes"},{class:Spirograph,name:"spirograph"}];function getTime(){return Date.now()}function updateAnimation(t,e=null){frames=0,avgDrawTime=0,avgElapsedTime=0,animationId=t,animation=new animations[animationId].class(canvas,colors,colorsAlt,bgColor),e&&animation.setSettings(e),then=getTime(),trueThen=then,animation.resize(),updateUI()}function updateAnimationResolution(t){"fit"!==(resolution=t)?(fixedWidth=parseInt(resolution.split("x")[0]),fixedHeight=parseInt(resolution.split("x")[1]),canvas.classList.add("fixed-size")):canvas.classList.remove("fixed-size")}function updateAnimationFps(t){fps=parseInt(t),framesInterval=1e3/fps}function checkResize(){"fit"===resolution?(width=Math.max(container.parentElement.offsetWidth-canvas.offsetLeft),height=Math.max(container.parentElement.offsetHeight-canvas.offsetTop),width===lastWidth&&height===lastHeight||(canvas.width=width,canvas.height=height,animation.resize())):canvas.width===fixedWidth&&canvas.height===fixedHeight||(canvas.width=fixedWidth,canvas.height=fixedHeight,animation.resize()),lastWidth=canvas.width,lastHeight=canvas.height}function updateStats(t,e){elemBgStats&&(++frames,avgElapsedTime=(avgElapsedTime*(sampleSize-1)+t)/sampleSize,avgDrawTime=(avgDrawTime*(sampleSize-1)+e)/sampleSize,frames%fps==0)&&(elemBgStats.innerHTML=`canvas resolution: ${canvas.width} x ${canvas.height}</br>
