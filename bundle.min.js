...
Coded with no external dependencies, using only canvas API.
`),this.lines=a,this.scale=r,this.speed=h,this.rainbowColors=n,this.radius=0}getVec(t){t=a.remap(t,0,this.lines,0,2*Math.PI);return a.rotateVec2d(a.createVec2d(this.radius,0),Math.PI+t)}draw(){this.clear(),this.radius=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/3*this.scale,this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2),a.strokeCircle(this.ctx,0,0,this.radius,this.colors[0]);for(let e=0;e<=this.lines;++e){var i=this.getVec(e),s=this.getVec(e*this.time);let t;t=this.rainbowColors?"hsl("+e/this.lines*360+", 100%, 75%)":a.lerpColorsPallet([this.colorA,this.colorB,this.colorA],e/this.lines),this.ctx.strokeStyle=t,a.drawLine(this.ctx,i.x,i.y,s.x,s.y,1,t)}this.ctx.resetTransform()}getSettings(){return[{prop:"lines",type:"int",min:1,max:2500},{prop:"speed",type:"float",min:-2,max:2},{prop:"scale",type:"float",min:.25,max:1.75},{prop:"rainbowColors",type:"bool"}]}}e.exports=r},{"./animation":3,"./utils":33}],5:[function(t,e,i){"use strict";var s=t("./animation");const h=t("./utils");class a extends s{constructor(t,e,i,s,a=180,r=.5,h=.4,n=1.2,o=.001,l=!1){super(t,e,i,s,"circular waves","circular-waves.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.numElements=r,this.elementPadding=h,this.cmpDuration=n,this.swapDuration=o,this.speed=l,this.showStats=c,this.sortAlgoNames=["selection sort","bubble sort","insertion sort","quick sort","merge sort","heap sort","gnome sort","shaker sort"],this.sortAlgoClasses=[p,d,m,f,g,x,v,w],this.sortingAlgorithm=this.assignIfRandom(a,u.randomChoice(this.sortAlgoNames)),this.initialOrderTypes=["random","sorted","reverse sorted","evens then odds","nearly sorted","few unique"],this.initialOrder="random",this.cmpTotal=0,this.cmpCount=0,this.accessTotal=0,this.accessCount=0,this.setup()}setup(){const i=this.numElements;this.animQueue=new o;let e=null;if(e="few unique"===this.initialOrder?Array.from({length:i},(t,e)=>(e%10+1)/10*i):Array.from({length:i},(t,e)=>e+1),["random","few unique"].includes(this.initialOrder)?u.randomShuffle(e,this.rand):"reverse sorted"===this.initialOrder?e=e.reverse():"evens then odds"===this.initialOrder&&(e=e.sort((t,e)=>t%2+t/(i+1)-(e%2+e/(i+1)))),"nearly sorted"===this.initialOrder){var s=Math.floor(i/5);for(let t=0;t<s;++t){var a=u.randomInt(0,i,this.rand),r=u.randomInt(0,i,this.rand);[e[a],e[r]]=[e[r],e[a]]}}this.elements=[];for(let t=0;t<i;++t){var h=e[t]/i,n=u.lerpColor(this.colors[0],this.colors[2],h);this.elements.push({val:h,pos:t,color:n,z:0})}var t=new this.sortAlgoClasses[this.sortAlgoNames.indexOf(this.sortingAlgorithm)](this.elements);this.moves=t.getMoves(),this.name=t.getName()+" algorithm visualization",this.timeComplexity=t.getTimeComplexity(),this.spaceComplexity=t.getSpaceComplexity(),this.cmpTotal=t.cmpCount,this.cmpCount=0,this.accessTotal=t.readCount+t.writeCount,this.accessCount=0}update(t){for(t=t/1e3*this.speed,this.time+=t,++this.frame;0<this.animQueue.step(t);){if(!this.moves.length)return;var h=this.moves[0];const n=t=>t<.5?u.easeInOutCubic(2*t):1-u.easeInOutCubic(2*t-1),o=u.easeInOutSine;if("cmp"===h[0]){let i=h[1],s=h[2];const a=i.color,r=s.color,l=this.colorsAlt[3],c=this.cmpDuration;++this.cmpCount,this.accessCount+=2,this.animQueue.push(function(t){var e=Math.min(t,c)/c;return i.color=u.lerpColor(a,l,n(e)),s.color=u.lerpColor(r,l,n(e)),t-c})}else if("swap"===h[0]){let i=h[1],e=h[2],s=[],a=[],r=[];const d=this.colorsAlt[1],p=this.frame,m=this.swapDuration*i.length;2===i.length?this.accessCount+=2:this.accessCount+=2*i.length;for(let t=0;t<i.length;++t)s.push(i[t].pos),a.push(e[t].pos),r.push(i[t].color);this.animQueue.push(function(t){var e=Math.min(t,m)/m;for(let t=0;t<i.length;++t)i[t].z=p,i[t].color=u.lerpColor(r[t],d,n(e)),i[t].pos=u.lerp(s[t],a[t],o(e));return t-m})}this.moves.shift()}}draw(){this.clear();var t,e,i=this.ctx.canvas.height,s=this.ctx.canvas.width/this.numElements;this.elements=this.elements.sort((t,e)=>t.z-e.z);for(t of this.elements){var a=t.pos*s+this.elementPadding/2,r=t.val*i;this.ctx.fillStyle=t.color,this.ctx.fillRect(a,0,s-this.elementPadding,r)}this.showStats&&(this.resetFont(),e=["Sorting algorithm: "+this.sortingAlgorithm,"Theoretical time complexity: "+this.timeComplexity,"Theoretical space complexity: "+this.spaceComplexity,"Number of elements: "+this.numElements,`Number of elements comparisons: ${this.cmpCount} / `+this.cmpTotal,`Number of array accesses (read/write): ${this.accessCount} / `+this.accessTotal],this.drawTextLines(e,this.lineHeight,this.ctx.canvas.height-(e.length+1)*this.lineHeight))}restart(){super.restart(),this.setup()}getSettings(){return[{prop:"initialOrder",type:"select",values:this.initialOrderTypes,toCall:"restart"},{prop:"sortingAlgorithm",type:"select",values:this.sortAlgoNames,toCall:"restart"},{prop:"numElements",label:"number of elements (n)",type:"int",min:8,max:256,toCall:"restart"},{prop:"speed",type:"float",step:.25,min:.5,max:8},{prop:"showStats",type:"bool"},this.getSeedSettings()]}}e.exports=r},{"./animation":3,"./animation-queue":2,"./utils":33}],31:[function(t,e,i){"use strict";var s=t("./animation");const d=t("./utils");class a extends s{constructor(t,e,i,s,a=500,r=0,h=!1,n=1,o=1,l="random",c=!1){super(t,e,i,s,"shapes dancing in a circle","spinning-shapes.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.shapeNames=["circles","points","lines","triangles","rectangles","pentagons","hexagons","heptagons","octagons"],this.vertices=this.assignIfRandom(r,d.randomInt(0,8)),this.updateName(),this.rotateShapes=h,this.shapes=a,this.distanceBase=.6,this.distanceRange=.2,this.sizeBase=.2,this.sizeRange=.12,this.scale=n,this.colorsScale=o,this.colorsShift=this.assignIfRandom(l,d.randomChoice([0,3.14])),this.rainbowColors=c}updateName(){this.name=this.shapeNames[this.vertices]+' "dancing" in a circle'}draw(){this.clear();var e=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/3*this.scale;this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2);for(let t=0;t<this.shapes;++t){var i=t/this.shapes*2*Math.PI,s=(this.distanceBase+this.distanceRange*Math.cos(6*i+Math.cos(8*i+this.time/2)))*e,a=Math.cos(i)*s,s=Math.sin(i)*s,r=9*i-this.time,h=(this.sizeBase+this.sizeRange*Math.cos(r))*e,r=(Math.cos((r+this.colorsShift)*this.colorsScale)+1)/2;this.rainbowColors?this.ctx.strokeStyle=`hsl(${360*r}, 100%, 75%)`:this.ctx.strokeStyle=d.lerpColor(this.colorA,this.colorB,r),this.ctx.lineWidth=1,this.ctx.beginPath(),0===this.vertices&&d.pathCircle(this.ctx,a,s,h),1===this.vertices?d.pathCircle(this.ctx,a,s,1):d.pathPolygon(this.ctx,a,s,h,this.vertices,i*this.rotateShapes),this.ctx.stroke()}this.ctx.resetTransform()}getSettings(){return[{prop:"vertices",type:"int",min:0,max:8,toCall:"updateName"},{prop:"shapes",type:"int",min:0,max:2500},{prop:"rotateShapes",type:"bool"},{prop:"distanceRange",type:"float",min:0,max:this.distanceBase},{prop:"sizeRange",type:"float",min:0,max:this.sizeBase},{prop:"scale",type:"float",min:.05,max:1.95},{prop:"speed",type:"float",step:.1,min:-4,max:4},{prop:"colorsShift",type:"float",min:0,max:3.14},{prop:"colorsScale",type:"float",min:.05,max:2},{prop:"rainbowColors",type:"bool"}]}}e.exports=a},{"./animation":3,"./utils":33}],32:[function(t,e,i){"use strict";var s=t("./animation");const l=t("./utils");class a extends s{constructor(t,e,i,s,a=4e3,r=2,h="random",n=!0,o=1){super(t,e,i,s,"spirograph","spirograph.js",`
