...
`),this.particlesSpeed=a,this.particlesDensity=r,this.noiseScale=n,this.fadingSpeed=o,this.nextFadeStep=0,this.particles=[],this.width=0,this.height=0}update(t){super.update(t),this.nextFadeStep=this.fadingSpeed*t/33;for(var e of this.particles){var i=2*this.noise.perlin3(e.x*this.noiseScale*2,e.y*this.noiseScale*3,this.frame*this.noiseScale*3)*Math.PI;e.x+=100*this.particlesSpeed*Math.tan(i)*t/1e3,e.y+=100*this.particlesSpeed*Math.sin(i)*t/1e3,e.x<0&&(e.x=this.width),e.x>this.width&&(e.x=0),e.y<0&&(e.y=this.height),e.y>this.height&&(e.y=0)}}draw(){this.fadeOut(this.nextFadeStep);for(var t of this.particles)this.ctx.fillStyle=t.color,this.ctx.fillRect(t.x,t.y,1,1)}resize(){this.clear(),this.width=this.canvas.width,this.height=this.canvas.height;var e=this.width*this.height*this.particlesDensity;this.particles=[];for(let t=0;t<e;t++){var i=this.rand()*this.width,s=this.rand()*this.height;this.particles.push({x:i,y:s,color:a.lerpColor(this.colorA,this.colorB,i/this.width)})}}updateColors(t,e,i){super.updateColors(t,e,i),this.resize()}getSettings(){return[{prop:"particlesSpeed",type:"float",step:.01,min:.1,max:2},{prop:"particlesDensity",type:"float",step:1e-4,min:1e-4,max:.05,toCall:"resize"},{prop:"noiseScale",type:"float",step:.001,min:.001,max:.01},{prop:"fadingSpeed",type:"float",step:.001,min:0,max:.1},this.getSeedSettings()]}}e.exports=r},{"../animation":2,"../utils":46}],20:[function(t,e,i){var s=t("../animation");let h=t("../utils");class a extends s{constructor(t,e,i,s,a=0,r,n=.002,o={x:"random",y:"random",z:1}){super(t,e,i,s,"visualization of Perlin noise","perlin-noise-grid.js",`
Grid of particles visualizing Perlin noise.

Coded with no external dependencies, using only canvas API.
`),this.pointsDensity=pointsDensity,this.drawPoints=drawPoints,this.maxPointsInNode=maxPointsInNode,this.noiseThreshold=noiseThreshold,this.drawLeafNodes=drawLeafNode,this.noiseScale=n,this.noiseSpeed=o,this.noisePos={x:0,y:0,z:0},this.noiseSpeed.x=this.assignIfRandom(this.noiseSpeed.x,h.round(h.randomRange(-1,1),1)),this.noiseSpeed.y=this.assignIfRandom(this.noiseSpeed.y,h.round(h.randomRange(-1,1),1)),this.minNodeSize=4,this.width=0,this.height=0,this.noisePos={x:0,y:0,z:0}}update(t){this.time+=t/1e3,++this.frame;let e=1,i=this.particlesSpeed;for(;1<i;)i/=2,e*=2;for(var s of this.particles)s.prevX=s.x,s.prevY=s.y;for(let t=0;t<e;++t)for(var a of this.particles){var r=2*this.noise.perlin2(a.x*this.noiseScale,a.y*this.noiseScale)*Math.PI/this.noiseScale;a.x+=Math.cos(r)*a.speed*i,a.y+=Math.sin(r)*a.speed*i}}draw(){this.fadeOut(this.fadingSpeed);for(var t of this.particles)h.drawLine(this.ctx,t.prevX,t.prevY,t.x,t.y,2*t.radius*this.particlesSize,t.color);this.imageData=this.ctx.getImageData(0,0,this.canvas.width,this.canvas.height)}restart(){super.restart(),this.clear(),this.particles=[],this.width=this.canvas.width,this.height=this.canvas.height,this.spawnParticles(0,0,this.width,this.height)}resize(){this.clear(),null!==this.imageData&&this.ctx.putImageData(this.imageData,0,0);var t=this.canvas.width-this.width,e=this.canvas.height-this.height;0<t&&this.spawnParticles(this.width,0,t,this.height),0<e&&this.spawnParticles(0,this.height,this.width,e),(0<t||0<e)&&this.spawnParticles(this.width,this.height,t,e),this.width=this.canvas.width,this.height=this.canvas.height;let i=this.width,s=this.height;this.particles=this.particles.filter(function(t){return!(t.x<0||t.x>i||t.y<0||t.y>s)})}updateColors(t,e,i){super.updateColors(t,e,i),this.restart()}getSettings(){return[{prop:"noiseScale",type:"float",step:.001,min:.001,max:.01,toCall:"restart"},{prop:"particlesDensity",type:"float",step:1e-4,min:1e-4,max:.005,toCall:"restart"},{prop:"particlesSpeed",type:"float",min:.25,max:32},{prop:"cellSize",type:"float",step:.1,min:1,max:4},{prop:"fadingSpeed",type:"float",step:1e-4,min:0,max:.01},this.getSeedSettings()]}}e.exports=a},{"../animation":2,"../utils":46}],21:[function(t,e,i){var s=t("../animation");let l=t("../utils");class a extends s{constructor(t,e,i,s,a=7e-4,r="random",n="random",o="random",h=0){super(t,e,i,s,"particles moving through Perlin noise","perlin-noise-particles.js",`
Particles moving through Perlin noise.

Coded by me (Marek Wydmuch) in 2020, with no external dependencies, using only canvas API.
`),this.particlesDensity=a,this.noiseScale=this.assignIfRandom(r,l.randomChoice([.001,.002,.003])),this.particlesSpeed=this.assignIfRandom(n,l.randomChoice([1,1.5,2])),this.particlesSize=this.assignIfRandom(o,l.randomChoice([1,1.5,2])),this.fadingSpeed=h,this.width=0,this.height=0,this.particles=[],this.imageData=null}update(t){var e,t=super.update(t);let i=1,s=this.particlesSpeed*t*1e3/30;for(;1<s;)s/=2,i*=2;for(e of this.particles)e.prevX=e.x,e.prevY=e.y;for(let t=0;t<i;++t)for(var a of this.particles){var r=2*this.noise.perlin2(a.x*this.noiseScale,a.y*this.noiseScale)*Math.PI/this.noiseScale;a.x+=Math.cos(r)*a.speed*s,a.y+=Math.sin(r)*a.speed*s}}draw(){this.fadeOut(this.fadingSpeed);for(var t of this.particles)l.drawLine(this.ctx,t.prevX,t.prevY,t.x,t.y,2*t.radius*this.particlesSize,t.color);this.imageData=this.ctx.getImageData(0,0,this.canvas.width,this.canvas.height)}spawnParticles(e,i,s,a){var r=s*a*this.particlesDensity;for(let t=0;t<r;t++){var n=this.rand()*s+e,o=this.rand()*a+i;this.particles.push({x:n,y:o,prevX:n,prevY:o,speed:.2*this.rand()+.1,radius:.5*this.rand()+.5,color:l.randomChoice(this.colors,this.rand)})}}restart(){super.restart(),this.clear(),this.particles=[],this.width=this.canvas.width,this.height=this.canvas.height,this.spawnParticles(0,0,this.width,this.height)}resize(){this.clear(),null!==this.imageData&&this.ctx.putImageData(this.imageData,0,0);var t=this.canvas.width-this.width,e=this.canvas.height-this.height;0<t&&this.spawnParticles(this.width,0,t,this.height),0<e&&this.spawnParticles(0,this.height,this.width,e),(0<t||0<e)&&this.spawnParticles(this.width,this.height,t,e),this.width=this.canvas.width,this.height=this.canvas.height;let i=this.width,s=this.height;this.particles=this.particles.filter(function(t){return!(t.x<0||t.x>i||t.y<0||t.y>s)})}updateColors(t,e,i){super.updateColors(t,e,i),this.restart()}getSettings(){return[{prop:"noiseScale",type:"float",step:.001,min:.001,max:.01,toCall:"restart"},{prop:"particlesDensity",type:"float",step:1e-4,min:1e-4,max:.005,toCall:"restart"},{prop:"particlesSpeed",type:"float",min:.25,max:32},{prop:"particlesSize",type:"float",step:.1,min:1,max:4},{prop:"fadingSpeed",type:"float",step:1e-4,min:0,max:.01},this.getSeedSettings()]}}e.exports=a},{"../animation":2,"../utils":46}],22:[function(t,e,i){var s=t("../animation");let d=t("../utils");class a extends s{constructor(t,e,i,s,a=1,r=.4,n=!1,o=.002,h={x:"random",y:"random",z:1},l=.01,c=!0){super(t,e,i,s,"quadtree visualization","quadtree.js",`
...
`)}}e.exports=s},{"../threejs-shader-animation":45}],40:[function(t,e,i){t=t("../shader-animation");class s extends t{constructor(t,e,i,s){super(t,e,i,s,`
    precision mediump float;
...
Just shader test animation.
`)}}e.exports=s},{"../shader-animation":38}],41:[function(t,e,i){t=t("./animation");class s extends t{constructor(t,e,i,s,a="",r="",n="",o="random"){super(t,e,i,s,a,r,n,o,null),this.renderer=new THREE.WebGLRenderer({antialias:!0,canvas:t}),this.renderer.setSize(t.width,t.height),this.scene=new THREE.Scene,this.camera=new THREE.PerspectiveCamera(75,t.width/t.height,1,1e3)}update(t){super.update(t)}draw(){this.renderer.render(this.scene,this.camera)}resize(){super.resize();var t=this.canvas.width,e=this.canvas.height;this.renderer.setSize(t,e),this.camera.aspect=t/e,this.camera.updateProjectionMatrix()}getCodeUrl(){return"https://github.com/mwydmuch/mwydmuch.github.io/blob/master/js/threejs-animations"+this.file}}e.exports=s},{"./animation":2}],42:[function(t,e,i){t=t("../threejs-animation");class s extends t{constructor(t,e,i,s){super(t,e,i,s,"Cubes animation","cubes.js",`
Just cubes animation.
`);t=new THREE.AmbientLight(2236962),t.position.set(0,0,0),this.scene.add(t),e=new THREE.PointLight(16777215,1),e.position.set(200,200,200),this.scene.add(e),this.cubeCount=1e3,i=new THREE.BoxGeometry,s=new THREE.MeshPhysicalMaterial;this.cubes=new THREE.InstancedMesh(i,s,this.cubeCount),this.cubes.instanceMatrix.setUsage(THREE.DynamicDrawUsage),this.scene.add(this.cubes)}update(t){super.update(t);var e=new THREE.Object3D;for(let t=0;t<this.cubeCount;++t)e.position.set(t,t,t),e.rotation.x=Math.sin(t+this.time),e.updateMatrix(),this.cubes.setMatrixAt(t,e.matrix);this.cubes.instanceMatrix.needsUpdate=!0}}e.exports=s},{"../threejs-animation":41}],43:[function(t,e,i){t=t("../threejs-animation");class s extends t{constructor(t,e,i,s){super(t,e,i,s,"Test Three.js animation","test.js",`
Just Three.js test animation.
`),this.scene.background=new THREE.Color(16777215),this.renderer=new THREE.WebGLRenderer({antialias:!0}),this.renderer.shadowMap.enabled=!0,this.renderer.shadowMap.type=THREE.PCFSoftShadowMap;t=new THREE.AmbientLight(4210752,.4),this.scene.add(t),e=new THREE.DirectionalLight(16777215,.8),e.position.set(5,8,5),e.castShadow=!0,e.shadow.camera.near=.1,e.shadow.camera.far=50,e.shadow.camera.left=-30,e.shadow.camera.right=30,e.shadow.camera.top=30,e.shadow.camera.bottom=-30,e.shadow.mapSize.width=4096,e.shadow.mapSize.height=4096,e.shadow.bias=-.001,this.scene.add(e),i=new THREE.DirectionalLight(8947967,.3),i.position.set(-5,3,-5),this.scene.add(i),s=new THREE.PlaneGeometry(200,200),t=new THREE.MeshPhongMaterial({color:8421504,shininess:60,specular:4210752,side:THREE.DoubleSide}),e=new THREE.Mesh(s,t);e.rotation.x=-Math.PI/2,e.position.y=-.01,e.receiveShadow=!0,this.scene.add(e),this.cubes=[],this.createGrid(),this.camera.position.set(0,4,12),this.camera.lookAt(0,0,-30),setInterval(floatRandomCube,1e3)}createGrid(){var i=new THREE.BoxGeometry(cubeSize,cubeSize,cubeSize),s=new THREE.MeshPhongMaterial({color:8421504,shininess:60,specular:4210752}),a=(this.cubes=[],gridSize*(cubeSize+gap)/2);for(let e=0;e<gridSize;e++){this.cubes[e]=[];for(let t=0;t<gridSize;t++){var r=new THREE.Mesh(i,s);r.position.set(e*(cubeSize+gap)-a,0,t*(cubeSize+gap)-a),r.castShadow=!0,r.receiveShadow=!0,this.scene.add(r),this.cubes[e][t]=r}}}floatRandomCube(){var t,e,i=Math.floor(Math.random()*gridSize),s=Math.floor(Math.random()*gridSize);this.cubes[i][s]&&(t=cubes[i][s],this.cubes[i][s]=null,e=new THREE.MeshPhongMaterial({color:8421504,shininess:60,specular:4210752,transparent:!0,opacity:1}),(e=new THREE.Mesh(t.geometry,e)).position.copy(t.position),e.castShadow=!0,e.receiveShadow=!0,this.scene.add(e),this.scene.remove(t),floatingCubes.push({mesh:e,startTime:Date.now(),startPos:e.position.clone(),gridX:i,gridZ:s}))}regenerateCube(t,e){var i=new THREE.BoxGeometry(cubeSize,cubeSize,cubeSize),s=new THREE.MeshPhongMaterial({color:8421504,shininess:60,specular:4210752});let a=new THREE.Mesh(i,s);a.castShadow=!0,a.receiveShadow=!0;i=gridSize*(cubeSize+gap)/2;a.position.set(t*(cubeSize+gap)-i,-cubeSize,e*(cubeSize+gap)-i),this.scene.add(a);let r=(this.cubes[t][e]=a).position.y,n=0,o=Date.now();!function t(){var e=Date.now()-o;e<1e3?(e/=1e3,a.position.y=r+(n-r)*e,requestAnimationFrame(t)):a.position.y=n}()}animate(){requestAnimationFrame(animate);let a=Date.now();floatingCubes=floatingCubes.filter(t=>{var e,i,s=a-t.startTime;return s<8e3?(i=t.startPos.y+20*(s/8e3),t.mesh.position.y=i,1e3<s&&(s=.02*Math.pow(.5+(s-1e3)/7e3,2),t.mesh.rotation.x+=s,t.mesh.rotation.y+=s),s=t.startPos.y,e=t.startPos.y+8,i=Math.max(0,1-(i-s)/(e-s)),!((t.mesh.material.opacity=i)<.01&&(scene.remove(t.mesh),regenerateCube(t.gridX,t.gridZ),1))):(scene.remove(t.mesh),regenerateCube(t.gridX,t.gridZ),!1)}),renderer.render(scene,camera)}update(t){super.update(t),this.scene.traverse(t=>{t.isMesh&&(t.rotation.x+=.01,t.rotation.y+=.01)})}}e.exports=s},{"../threejs-animation":41}],44:[function(t,e,i){t=t("../threejs-animation");class s extends t{constructor(t,e,i,s){super(t,e,i,s,"Test Three.js animation","test.js",`
Just Three.js test animation.
`);t=new THREE.AmbientLight(2236962),t.position.set(0,0,0),this.scene.add(t),e=new THREE.PointLight(16777215,1),e.position.set(200,200,200),this.scene.add(e),i=new THREE.BoxGeometry,s=new THREE.MeshPhysicalMaterial,t=new THREE.Mesh(i,s);t.position.set(0,0,0),this.scene.add(t)}update(t){super.update(t),this.scene.traverse(t=>{t.isMesh&&(t.rotation.x+=.01,t.rotation.y+=.01)})}}e.exports=s},{"../threejs-animation":41}],45:[function(t,e,i){t=t("./threejs-animation");class s extends t{constructor(t,e,i,s,a,r="",n="",o="",h="random"){super(t,e,i,s,r,n,o,h,"webgl");this.camera=new THREE.OrthographicCamera(-1,1,1,-1,.1,10),this.camera.position.z=1,this.shaderMaterial=new THREE.ShaderMaterial({vertexShader:`
            varying vec2 vUv;
