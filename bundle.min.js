...
Coded with no external dependencies, using only canvas API.
`),this.shapeSides=[0,1,2,3,4,5,6,8],this.shapeNames=["circles","points","lines","triangles","rectangles","pentagons","hexagons","octagons"],this.sides=this.assignIfRandom(h,n.randomChoice(this.shapeSides)),this.updateName(),this.size=r,this.number=a}updateName(){this.name=this.shapeNames[this.shapeSides.indexOf(this.sides)]+" spinning in spiral"}draw(){this.clear(),this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2),this.ctx.scale(this.scale,this.scale),this.ctx.strokeStyle=this.colors[0];for(let t=0;t<this.number;++t)this.ctx.rotate(Math.PI*(.001*this.time+1e-6*t)),this.ctx.beginPath(),0===this.sides&&n.pathCircle(this.ctx,t,t,this.size),1===this.sides?n.pathCircle(this.ctx,t,t,1):n.pathPolygon(this.ctx,t,t,this.size,this.sides,0),this.ctx.stroke();this.ctx.resetTransform()}getSettings(){return[{prop:"sides",type:"int",min:0,max:8,toCall:"updateName"},{prop:"number",type:"int",min:1,max:1024},{prop:"size",type:"int",min:1,max:128},{prop:"speed",type:"float",step:.1,min:-4,max:4}]}}e.exports=a},{"./animation":3,"./utils":36}],9:[function(t,e,i){"use strict";var s=t("./game-of-life");const p=t("./utils");class a extends s{constructor(t,e,i,s,a=14,r=0,h=.5,n=!0,o=!0,l=!0,c=.75){super(t,e,i,s,a,r,h,l),this.name="isometric Conway's game of life",this.file="game-of-life-isometric.js",this.description=`
...
Coded with no external dependencies, using only canvas API.
`),this.noise=d.noise,this.noise.seed(p.randomRange(0,1)),this.particles=a,this.radiusMin=50,this.radiusMax=250,this.radius=this.assignIfRandom(r,p.round(p.randomRange(this.radiusMin,this.radiusMax),2)),this.speedMin=.02,this.speedMax=.05,this.speed=this.assignIfRandom(h,p.round(p.randomRange(this.speedMin,this.speedMax)*p.randomChoice([-1,1]),2)),this.rotationSpeedMin=.01,this.rotationSpeedMax=.02,this.rotationSpeed=this.assignIfRandom(n,p.round(p.randomRange(this.rotationSpeedMin,this.rotationSpeedMax)*p.randomChoice([-1,1]),2)),this.dirMax=.75,this.dirX=this.assignIfRandom(o,p.round(p.randomRange(-this.dirMax,this.dirMax),2)),this.dirY=this.assignIfRandom(l,p.round(p.randomRange(-this.dirMax,this.dirMax),2)),this.scale=c}draw(){this.clear(),this.ctx.lineWidth=1,this.ctx.strokeStyle=this.colors[0];var t=Math.min(this.ctx.canvas.width,this.ctx.canvas.height)/4,e=this.ctx.canvas.width/2+this.dirX*t,t=this.ctx.canvas.height/2+this.dirY*t,i=Math.round(1e4*this.time)/10;this.ctx.translate(e,t),this.ctx.scale(this.scale,this.scale),this.ctx.beginPath();for(let t=1;t<=this.particles;t++){var s=this.radius+Math.pow(t/(this.particles/1.5),2)*t/2,a=100*this.noise.perlin2(.1*t+i,.1)+i*this.rotationSpeed,r=Math.cos(a)*s+Math.sqrt(t*this.radius)*this.dirX,a=Math.sin(a)*s+Math.sqrt(t*this.radius)*this.dirY;p.pathCircle(this.ctx,r,a,.01*t)}this.ctx.stroke(),this.ctx.resetTransform()}getSettings(){return[{prop:"particles",type:"int",min:1,max:3e3},{prop:"radius",type:"float",min:this.radiusMin,max:this.radiusMax},{prop:"speed",type:"float",min:-this.speedMax,max:this.speedMax},{prop:"rotationSpeed",type:"float",min:-this.rotationSpeedMax,max:this.rotationSpeedMax},{prop:"dirX",type:"float",min:-this.dirMax,max:this.dirMax},{prop:"dirY",type:"float",min:-this.dirMax,max:this.dirMax}]}}e.exports=a},{"./animation":3,"./noise":19,"./utils":36}],23:[function(t,e,i){"use strict";var s=t("./animation");const a=t("./utils");class r extends s{constructor(t,e,i,s,a=.01,r=.001,h=.02){super(t,e,i,s,"particles waves","particles-waves.js",`
"Particles waves" animation.
The effect was achieved by modifying Perlin noise animation.

Coded with no external dependencies, using only canvas API.
`),this.particlesDensity=a,this.noiseScale=r,this.fadingSpeed=h,this.particles=[],this.width=0,this.height=0}update(t){super.update(t);for(var e of this.particles){var i=2*this.noise.perlin3(e.x*this.noiseScale*2,e.y*this.noiseScale*3,this.frame*this.noiseScale*3)*Math.PI;e.x+=2*Math.tan(i),e.y+=2*Math.sin(i),e.x<0&&(e.x=this.width),e.x>this.width&&(e.x=0),e.y<0&&(e.y=this.height),e.y>this.height&&(e.y=0)}}draw(){this.fadeOut(this.fadingSpeed);for(var t of this.particles)this.ctx.fillStyle=t.color,this.ctx.fillRect(t.x,t.y,1,1)}resize(){this.clear(),this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height;var e=this.width*this.height*this.particlesDensity;this.particles=[];for(let t=0;t<e;t++){var i=this.rand()*this.width,s=this.rand()*this.height;this.particles.push({x:i,y:s,color:a.lerpColor(this.colorA,this.colorB,i/this.width)})}}getSettings(){return[{prop:"particlesDensity",type:"float",step:1e-4,min:1e-4,max:.05,toCall:"resize"},{prop:"noiseScale",type:"float",step:.001,min:.001,max:.01},{prop:"fadingSpeed",type:"float",step:.001,min:0,max:.1},this.getSeedSettings()]}}e.exports=r},{"./animation":3,"./utils":36}],24:[function(t,e,i){"use strict";var s=t("./animation");const l=t("./noise"),c=t("./utils");class a extends s{constructor(t,e,i,s,a=6e-4,r=.001,h=1,n=1,o=0){super(t,e,i,s,"particles moving through Perlin noise","perlin-noise-particles.js",`
Particles moving through Perlin noise.

Coded with no external dependencies, using only canvas API.
`),this.particlesDensity=a,this.noiseScale=r,this.noise=l.noise,this.noise.seed(c.randomRange(0,1)),this.particlesSpeed=h,this.particlesSize=n,this.fadingSpeed=o,this.width=0,this.height=0,this.particles=[],this.imageData=null}update(t){this.time+=t/1e3,++this.frame;let e=1,i=this.particlesSpeed;for(;1<i;)i/=2,e*=2;for(var s of this.particles)s.prevX=s.x,s.prevY=s.y;for(let t=0;t<e;++t)for(var a of this.particles){var r=2*this.noise.perlin2(a.x*this.noiseScale,a.y*this.noiseScale)*Math.PI/this.noiseScale;a.x+=Math.cos(r)*a.speed*i,a.y+=Math.sin(r)*a.speed*i}}draw(){this.fadeOut(this.fadingSpeed);for(var t of this.particles)c.drawLine(this.ctx,t.prevX,t.prevY,t.x,t.y,2*t.radius*this.particlesSize,t.color);this.imageData=this.ctx.getImageData(0,0,this.ctx.canvas.width,this.ctx.canvas.height)}spawnParticles(e,i,s,a){var r=s*a*this.particlesDensity;for(let t=0;t<r;t++){var h=this.rand()*s+e,n=this.rand()*a+i;this.particles.push({x:h,y:n,prevX:h,prevY:n,speed:.2*this.rand()+.1,radius:.5*this.rand()+.5,color:c.randomChoice(this.colors)})}}restart(){this.clear(),this.particles=[],this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height,this.spawnParticles(0,0,this.width,this.height),super.restart()}resize(){this.clear(),null!==this.imageData&&this.ctx.putImageData(this.imageData,0,0);var t=this.ctx.canvas.width-this.width,e=this.ctx.canvas.height-this.height;0<t&&this.spawnParticles(this.width,0,t,this.height),0<e&&this.spawnParticles(0,this.height,this.width,e),(0<t||0<e)&&this.spawnParticles(this.width,this.height,t,e),this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height;const i=this.width,s=this.height;this.particles=this.particles.filter(function(t){return!(t.x<0||t.x>i||t.y<0||t.y>s)})}getSettings(){return[{prop:"noiseScale",type:"float",step:.001,min:.001,max:.01,toCall:"retart"},{prop:"particlesDensity",type:"float",step:1e-4,min:1e-4,max:.005,toCall:"restart"},{prop:"particlesSpeed",type:"float",min:.25,max:32},{prop:"particlesSize",type:"float",step:.1,min:1,max:4},{prop:"fadingSpeed",type:"float",step:1e-4,min:0,max:.01},this.getSeedSettings()]}}e.exports=a},{"./animation":3,"./noise":19,"./utils":36}],25:[function(t,e,i){"use strict";var s=t("./animation");const d=t("./utils");class a extends s{constructor(t,e,i,s,a=1,r=.4,h=!1,n=.002,o={x:"random",y:"random",z:1},l=.01,c=!0){super(t,e,i,s,"quadtree visualization","quadtree.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.shapeNames=["circles","points","lines","triangles","rectangles","pentagons","hexagons","heptagons","octagons"],this.vertices=this.assignIfRandom(r,d.randomInt(0,8)),this.updateName(),this.rotateShapes=h,this.shapes=a,this.distanceBase=.6,this.distanceRange=.2,this.sizeBase=.2,this.sizeRange=.12,this.scale=n,this.colorsScale=o,this.colorsShift=this.assignIfRandom(l,d.randomChoice([0,3.14])),this.rainbowColors=c}updateName(){this.name=this.shapeNames[this.vertices]+' "dancing" in a circle'}draw(){this.clear();var e=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/3*this.scale;this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2);for(let t=0;t<this.shapes;++t){var i=t/this.shapes*2*Math.PI,s=(this.distanceBase+this.distanceRange*Math.cos(6*i+Math.cos(8*i+this.time/2)))*e,a=Math.cos(i)*s,s=Math.sin(i)*s,r=9*i-this.time,h=(this.sizeBase+this.sizeRange*Math.cos(r))*e,r=(Math.cos((r+this.colorsShift)*this.colorsScale)+1)/2;this.rainbowColors?this.ctx.strokeStyle=`hsl(${360*r}, 100%, 75%)`:this.ctx.strokeStyle=d.lerpColor(this.colorA,this.colorB,r),this.ctx.lineWidth=1,this.ctx.beginPath(),0===this.vertices&&d.pathCircle(this.ctx,a,s,h),1===this.vertices?d.pathCircle(this.ctx,a,s,1):d.pathPolygon(this.ctx,a,s,h,this.vertices,i*this.rotateShapes),this.ctx.stroke()}this.ctx.resetTransform()}getSettings(){return[{prop:"vertices",type:"int",min:0,max:8,toCall:"updateName"},{prop:"shapes",type:"int",min:0,max:2500},{prop:"rotateShapes",type:"bool"},{prop:"distanceRange",type:"float",min:0,max:this.distanceBase},{prop:"sizeRange",type:"float",min:0,max:this.sizeBase},{prop:"scale",type:"float",min:.05,max:1.95},{prop:"speed",type:"float",step:.1,min:-4,max:4},{prop:"colorsShift",type:"float",min:0,max:3.14},{prop:"colorsScale",type:"float",min:.05,max:2},{prop:"rainbowColors",type:"bool"}]}}e.exports=a},{"./animation":3,"./utils":36}],34:[function(t,e,i){"use strict";var s=t("./animation");const l=t("./utils");class a extends s{constructor(t,e,i,s,a=2500,r=2,h="random",n=!0,o=1){super(t,e,i,s,"spirograph","spirograph.js",`
