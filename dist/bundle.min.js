...
Coded with no external dependencies, using only canvas API.
`),this.lines=s,this.scale=a,this.speed=r,this.rainbowColors=h,this.radius=0}getVec(t){t=a.remap(t,0,this.lines,0,2*Math.PI);return a.rotateVec2d(a.createVec2d(this.radius,0),Math.PI+t)}draw(){this.clear(),this.radius=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/3*this.scale,this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2),a.strokeCircle(this.ctx,0,0,this.radius,this.colors[0]);for(let e=0;e<=this.lines;++e){var i=this.getVec(e),s=this.getVec(e*this.time);let t;t=this.rainbowColors?"hsl("+e/this.lines*360+", 100%, 75%)":a.lerpColorsPallet([this.colorA,this.colorB,this.colorA],e/this.lines),a.drawLine(this.ctx,i.x,i.y,s.x,s.y,t,1)}this.ctx.resetTransform()}getSettings(){return[{prop:"lines",type:"int",min:1,max:2500},{prop:"speed",type:"float",min:-2,max:2},{prop:"scale",type:"float",min:.25,max:1.75},{prop:"rainbowColors",type:"bool"}]}}e.exports=r},{"./animation":2,"./utils":27}],4:[function(t,e,i){"use strict";var s=t("./animation");const l=t("./noise"),c=t("./utils");class a extends s{constructor(t,e,i,s=180,a=.5,r=.4,h=1.2,n=.001,o=!1){super(t,e,i,"circular waves","circular-waves.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.noise=l.noise,this.noise.seed(c.randomRange(0,1)),this.vertices=s,this.noiseScale=a,this.radiusScaleMin=r,this.radiusScaleMax=h,this.fadingSpeed=n,this.rainbowColors=o,this.radiusMin=0,this.radiusMax=0}draw(){this.fadeOut(this.fadingSpeed);var e=.005*this.frame,i=2*Math.PI/this.vertices;this.rainbowColors?this.ctx.strokeStyle="hsl("+360*Math.abs(Math.sin(5*e))+", 100%, 50%)":this.ctx.strokeStyle=c.lerpColor(this.colorA,this.colorB,Math.abs(Math.sin(5*e))),this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2),this.ctx.beginPath();for(let t=0;t<=this.vertices;++t){var s=t*i,a=Math.cos(s)*this.noiseScale,r=Math.sin(s)*this.noiseScale,a=this.noise.simplex3(a,r,e),r=c.remap(a,-1,1,this.radiusMin,this.radiusMax),a=r*Math.cos(s),r=r*Math.sin(s);0===t?this.ctx.moveTo(a,r):this.ctx.lineTo(a,r)}this.ctx.stroke(),this.ctx.resetTransform()}resize(){this.radiusMin=Math.min(this.ctx.canvas.width,this.ctx.canvas.height)/2*this.radiusScaleMin,this.radiusMax=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/2*this.radiusScaleMax,this.radiusMin>this.radiusMax&&([this.radiusMin,this.radiusMax]=[this.radiusMax,this.radiusMin]),this.clear()}getSettings(){return[{prop:"vertices",type:"int",min:3,max:720,toCall:"resize"},{prop:"radiusScaleMin",type:"float",min:0,max:2,toCall:"resize"},{prop:"radiusScaleMax",type:"float",min:0,max:2,toCall:"resize"},{prop:"noiseScale",type:"float",min:0,max:2,toCall:"resize"},{prop:"fadingSpeed",type:"float",step:.001,min:0,max:.1},{prop:"rainbowColors",type:"bool"}]}}e.exports=a},{"./animation":2,"./noise":15,"./utils":27}],5:[function(t,e,i){"use strict";var s=t("./animation");const r=t("./utils");class a extends s{constructor(t,e,i,s=20,a=4){super(t,e,i,"Code writing animation","codding.js",`
Work in progress.
...
Coded with no external dependencies, using only canvas API.
`),this.dropsSize=s,this.dropsSpeed=a,this.fadingSpeed=r,this.originalMatrixColors=h,this.flipProp=.25,this.errorProp=.1,this.cellWidth=0,this.cellHeight=0,this.columns=0,this.columnHeight=0,this.drops=[],this.textColor=null,this.imageData=null,this.setColors();this.characters='ｦｰｱｲｳｴｵｶｷｸｹｺｻｼｽｾｿﾀﾁﾂﾃﾄﾅﾆﾇﾈﾉﾊﾋﾌﾍﾎﾏﾐﾑﾒﾓﾔﾕﾖﾗﾘﾙﾚﾜﾝ0123456789*+:=.<>#@!?^~"'}dropSpawnPoint(t){return a.randomInt(0,Math.min(t-1,this.columnHeight/2))-1}dropDespawn(t){return Math.random()<.1*Math.pow(t/this.columnHeight,2)||t>this.columnHeight}drawCharacter(t,e,i,s){this.ctx.fillStyle=this.bgColor,this.ctx.fillRect(e-this.cellWidth/2,i,this.cellWidth,this.cellHeight),this.ctx.fillStyle=s,Math.random()<this.flipProp?(this.ctx.save(),this.ctx.translate(e,i),this.ctx.scale(-1,1),this.ctx.fillText(t,0,0),this.ctx.restore()):this.ctx.fillText(t,e,i)}setColors(){this.originalMatrixColors?(this.bgColor="#000000",this.textColor="#00FF00"):(this.bgColor="#FFFFFF",this.textColor=this.colors[0])}draw(){this.originalMatrixColors?this.blendColorAlpha(this.bgColor,this.fadingSpeed,"darken"):this.blendColorAlpha(this.bgColor,this.fadingSpeed,"lighter"),this.ctx.font=this.dropsSize+"px monospace",this.ctx.textAlign="center",this.ctx.textBaseline="top";for(var t of this.drops){var e,i;Math.floor(t.y)!==Math.floor(t.y+this.dropsSpeed)?(t.y+=this.dropsSpeed,e=t.x*this.cellWidth+this.cellWidth/2,i=Math.floor(t.y)*this.cellHeight,this.drawCharacter(t.char,e,i,this.textColor),t.char=a.randomChoice(this.characters),this.dropDespawn(t.y)&&(t.y=this.dropSpawnPoint(t.y)),Math.random()<this.errorProp&&(i=a.randomInt(-8,8),this.drawCharacter(a.randomChoice(this.characters),e,Math.floor(i+t.y)*this.cellHeight,this.textColor))):t.y+=this.dropsSpeed}this.imageData=this.ctx.getImageData(0,0,this.ctx.canvas.width,this.ctx.canvas.height)}resize(){if(this.clear(),null!==this.imageData&&this.ctx.putImageData(this.imageData,0,0),this.cellHeight=this.dropsSize,this.cellWidth=Math.ceil(this.dropsSize/1.618),this.columns=this.ctx.canvas.width/this.cellWidth,this.columnHeight=this.ctx.canvas.height/this.cellHeight,this.drops.length<this.columns)for(let t=this.drops.length;t<this.columns;++t)this.drops.push({char:a.randomChoice(this.characters),x:t,y:this.dropSpawnPoint(this.columnHeight)})}restart(){this.drops=[],this.setColors(),this.resize(),this.clear()}getSettings(){return[{prop:"dropsSize",type:"int",min:8,max:64,toCall:"resize"},{prop:"dropsSpeed",type:"float",min:0,max:1},{prop:"fadingSpeed",type:"float",step:.001,min:0,max:.5}]}}e.exports=r},{"./animation":2,"./utils":27}],13:[function(t,e,i){"use strict";var s=t("./animation");const h=t("./utils"),r=t("./delaunay");class a extends s{constructor(t,e,i,s=2e-4,a=!0,r=!1,h=125){super(t,e,i,"Delaunay triangulation for a cloud of particles","network.js",`
...
`),this.particlesDensity=s,this.fillTriangles=a,this.drawParticles=r,this.distanceThreshold=h,this.width=0,this.height=0,this.particles=[]}drawTriangle(t,e,i){var s=Math.max(h.distVec2d(t,e),h.distVec2d(t,e),h.distVec2d(e,i));s>this.distanceThreshold||(this.ctx.beginPath(),h.pathClosedShape(this.ctx,[t,e,i]),e=h.lerpColor(t.color,this.bgColor,h.easeInSine(s/this.distanceThreshold)),this.fillTriangles?(this.ctx.fillStyle=e,this.ctx.fill()):(this.ctx.lineWidth=1,this.ctx.strokeStyle=e,this.ctx.stroke()))}update(t){super.update(t);for(var e of this.particles)e.x+=e.velX*this.speed,e.y+=e.velY*this.speed,(e.x<0||e.x>this.width)&&(e.velX*=-1),(e.y<0||e.y>this.height)&&(e.velY*=-1)}draw(){if(this.clear(),0<this.particles.length){var e=r.triangulate(this.particles.map(function(t){return[t.x,t.y]}));for(let t=0;t<e.length;t+=3){var i=this.particles[e[t]],s=this.particles[e[t+1]],a=this.particles[e[t+2]];this.drawTriangle(i,s,a)}}if(this.drawParticles)for(var t of this.particles)h.fillCircle(this.ctx,t.x,t.y,2,t.color)}spawnParticles(e,i,s,a){var r=s*a*this.particlesDensity;for(let t=0;t<r;t++)this.particles.push({x:Math.random()*s+e,y:Math.random()*a+i,velY:2*Math.random()-1,velX:2*Math.random()-1,color:h.randomChoice(this.colors)})}restart(){this.particles=[],this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height,this.spawnParticles(0,0,this.width,this.height)}resize(){var t=this.ctx.canvas.width-this.width,e=this.ctx.canvas.height-this.height;0<t&&this.spawnParticles(this.width,0,t,this.height),0<e&&this.spawnParticles(0,this.height,this.width,e),(0<t||0<e)&&this.spawnParticles(this.width,this.height,t,e),this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height;const i=this.width,s=this.height;this.particles=this.particles.filter(function(t){return!(t.x<0||t.x>i||t.y<0||t.y>s)})}getSettings(){return[{prop:"particlesDensity",type:"float",step:1e-4,min:1e-4,max:.002,toCall:"restart"},{prop:"fillTriangles",type:"bool"},{prop:"drawParticles",type:"bool"},{prop:"distanceThreshold",type:"int",min:0,max:200},{prop:"speed",type:"float",step:.1,min:-4,max:4}]}}e.exports=a},{"./animation":2,"./delaunay":6,"./utils":27}],14:[function(t,e,i){"use strict";var s=t("./animation");const c=t("./utils");class a extends s{constructor(t,e,i){super(t,e,i,"visualization of simple neural network","neural-network.js"),this.network=[],this.nLayers=0,this.baseNodeSize=3,this.baseLineSize=1,this.fps=1.5}update(t){if(0!==this.network.length){for(var e of this.network[0])e.v=c.randomRange(-1,1);for(let e=1;e<this.nLayers;e++)for(var i of this.network[e]){for(let t=i.v=0;t<this.network[e-1].length;++t)i.v+=this.network[e-1][t].v*i.w[t];e===this.nLayers-1?i.nlv=1/(1+Math.exp(-i.v)):i.nlv=Math.max(0,i.v)}}}draw(){this.clear();for(let t=0;t<this.nLayers-1;t++){var e,i=this.network[t],s=this.network[t+1];for(e of i)for(var a of s){var r=c.clip(e.v,0,1),h=this.colors[this.colors.length-1-Math.floor(r*this.colors.length)];this.ctx.globalAlpha=r,c.drawLine(this.ctx,e.x,e.y,a.x,a.y,h,1+r)}}this.ctx.globalAlpha=1;for(let e=0;e<this.nLayers;++e){var n;for(n of this.network[e]){var o=c.clip(n.nlv,0,1),l=c.clip(2*n.nlv,0,4),o=this.colors[this.colors.length-1-Math.floor(o*this.colors.length)],l=this.baseNodeSize+l;c.fillCircle(this.ctx,n.x,n.y,l,o),this.ctx.font="12px sans-serif",this.ctx.textAlign="center";let t=`ReLU(${c.round(n.v,2)}) = `+c.round(n.nlv,2);0===e?t=""+c.round(n.v,2):e===this.nLayers-1&&(t=`Sigmoid(${c.round(n.v,2)}) = `+c.round(n.nlv,2)),this.ctx.fillText(t,n.x,n.y-3*this.baseNodeSize)}}}resize(){c.clear(this.ctx,"#FFFFFF");var t=this.ctx.canvas.width,a=this.ctx.canvas.height,e=(this.network=[],this.nLayers=c.clip(Math.floor(t/150),3,7),50*t/500),r=(t-2*e)/(this.nLayers-1),h=a/17;let n=e;for(let s=0;s<this.nLayers;s++){var o=[];let e=0,i=(e=0==s||s==this.nLayers-1?Math.floor(c.randomRange(4,16)):c.randomChoice([8,12,16]),a/2-Math.floor(e/2)*h);e%2==0&&(i+=h/2);for(let t=0;t<e;t++){var l={x:n,y:i,v:0,nlv:0,w:null};0<s&&(l.w=c.randomArray(this.network[s-1].length,-1,1)),o.push(l),i+=h}this.network.push(o),n+=r}this.update(0),this.draw()}getSettings(){return[]}}e.exports=a},{"./animation":2,"./utils":27}],15:[function(t,e,i){"use strict";var s=this;function a(t,e,i){this.x=t,this.y=e,this.z=i}s=s.noise={},a.prototype.dot2=function(t,e){return this.x*t+this.y*e},a.prototype.dot3=function(t,e,i){return this.x*t+this.y*e+this.z*i};var r=[new a(1,1,0),new a(-1,1,0),new a(1,-1,0),new a(-1,-1,0),new a(1,0,1),new a(-1,0,1),new a(1,0,-1),new a(-1,0,-1),new a(0,1,1),new a(0,-1,1),new a(0,1,-1),new a(0,-1,-1)],h=[151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,21,10,23,190,6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,88,237,149,56,87,174,20,125,136,171,168,68,175,74,165,71,134,139,48,27,166,77,146,158,231,83,111,229,122,60,211,133,230,220,105,92,41,55,46,245,40,244,102,143,54,65,25,63,161,1,216,80,73,209,76,132,187,208,89,18,169,200,196,135,130,116,188,159,86,164,100,109,198,173,186,3,64,52,217,226,250,124,123,5,202,38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,182,189,28,42,223,183,170,213,119,248,152,2,44,154,163,70,221,153,101,155,167,43,172,9,129,22,39,253,19,98,108,110,79,113,224,232,178,185,112,104,218,246,97,228,251,34,242,193,238,210,144,12,191,179,162,241,81,51,145,235,249,14,239,107,49,192,214,31,181,199,106,157,184,84,204,176,115,121,50,45,127,4,150,254,138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180],C=new Array(512),M=new Array(512),d=(s.seed=function(t){0<t&&t<1&&(t*=65536),(t=Math.floor(t))<256&&(t|=t<<8);for(var e=0;e<256;e++){var i=1&e?h[e]^255&t:h[e]^t>>8&255;C[e]=C[e+256]=i,M[e]=M[e+256]=r[i%12]}},s.seed(0),.5*(Math.sqrt(3)-1)),p=(3-Math.sqrt(3))/6,b=1/6;function m(t){return t*t*t*(t*(6*t-15)+10)}function u(t,e,i){return(1-i)*t+i*e}s.simplex2=function(t,e){var i=(t+e)*d,s=Math.floor(t+i),i=Math.floor(e+i),a=(s+i)*p,t=t-s+a,e=e-i+a,a=e<t?(c=1,0):(c=0,1),r=t-c+p,h=e-a+p,n=t-1+2*p,o=e-1+2*p,l=M[(s&=255)+C[i&=255]],c=M[s+c+C[i+a]],a=M[1+s+C[1+i]],s=.5-t*t-e*e,i=s<0?0:(s*=s)*s*l.dot2(t,e),s=.5-r*r-h*h,l=s<0?0:(s*=s)*s*c.dot2(r,h),t=.5-n*n-o*o,e=t<0?0:(t*=t)*t*a.dot2(n,o);return 70*(i+l+e)},s.simplex3=function(t,e,i){var s=(t+e+i)*(1/3),a=Math.floor(t+s),r=Math.floor(e+s),s=Math.floor(i+s),h=(a+r+s)*b,t=t-a+h,e=e-r+h,i=i-s+h,h=e<=t?i<=e?(S=w=x=1,y=v=0):w=i<=t?(S=y=v=0,x=1):(S=v=x=0,y=1):e<i?(w=v=x=0,S=y=1):t<i?(w=y=x=0,S=v=1):(S=w=v=1,y=x=0),n=t-x+b,o=e-v+b,l=i-y+b,c=t-w+2*b,d=e-S+2*b,p=i-h+2*b,m=t-1+.5,u=e-1+.5,g=i-1+.5,f=M[(a&=255)+C[(r&=255)+C[s&=255]]],x=M[a+x+C[r+v+C[s+y]]],v=M[a+w+C[r+S+C[s+h]]],y=M[1+a+C[1+r+C[1+s]]],w=.6-t*t-e*e-i*i,S=w<0?0:(w*=w)*w*f.dot3(t,e,i),h=.6-n*n-o*o-l*l,a=h<0?0:(h*=h)*h*x.dot3(n,o,l),r=.6-c*c-d*d-p*p,s=r<0?0:(r*=r)*r*v.dot3(c,d,p),w=.6-m*m-u*u-g*g,f=w<0?0:(w*=w)*w*y.dot3(m,u,g);return 32*(S+a+s+f)},s.perlin2=function(t,e){var i=Math.floor(t),s=Math.floor(e),a=(t-=i,e-=s,M[(i&=255)+C[s&=255]].dot2(t,e)),r=M[i+C[1+s]].dot2(t,e-1),h=M[1+i+C[s]].dot2(t-1,e),i=M[1+i+C[1+s]].dot2(t-1,e-1),s=m(t);return u(u(a,h,s),u(r,i,s),m(e))},s.perlin3=function(t,e,i){var s=Math.floor(t),a=Math.floor(e),r=Math.floor(i),h=(t-=s,e-=a,i-=r,M[(s&=255)+C[(a&=255)+C[r&=255]]].dot3(t,e,i)),n=M[s+C[a+C[1+r]]].dot3(t,e,i-1),o=M[s+C[1+a+C[r]]].dot3(t,e-1,i),l=M[s+C[1+a+C[1+r]]].dot3(t,e-1,i-1),c=M[1+s+C[a+C[r]]].dot3(t-1,e,i),d=M[1+s+C[a+C[1+r]]].dot3(t-1,e,i-1),p=M[1+s+C[1+a+C[r]]].dot3(t-1,e-1,i),s=M[1+s+C[1+a+C[1+r]]].dot3(t-1,e-1,i-1),a=m(t),r=m(e),t=m(i);return u(u(u(h,c,a),u(n,d,a),t),u(u(o,p,a),u(l,s,a),t),r)}},{}],16:[function(t,e,i){"use strict";var s=t("./animation");const m=t("./utils");class a extends s{constructor(t,e,i,s=1e4,a="random",r=.03,h=5,n="random",o="random",l="random",c=!1,d=1,p=!1){super(t,e,i,"system of particles and attractors","particles-and-attractors.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.shapeNames=["circles","points","lines","triangles","rectangles","pentagons","hexagons","heptagons","octagons"],this.vertices=this.assignIfRandom(a,c.randomInt(0,8)),this.updateName(),this.rotateShapes=r,this.shapes=s,this.distanceBase=.6,this.distanceRange=.2,this.sizeBase=.2,this.sizeRange=.12,this.scale=h,this.colorsScale=n,this.colorsShift=this.assignIfRandom(o,c.randomChoice([0,3.14])),this.rainbowColors=l}updateName(){this.name=this.shapeNames[this.vertices]+' "dancing" in a circle'}draw(){this.clear();var e=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/3*this.scale;this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2);for(let t=0;t<this.shapes;++t){var i=t/this.shapes*2*Math.PI,s=(this.distanceBase+this.distanceRange*Math.cos(6*i+Math.cos(8*i+this.time/2)))*e,a=Math.cos(i)*s,s=Math.sin(i)*s,r=9*i-this.time,h=(this.sizeBase+this.sizeRange*Math.cos(r))*e,r=(Math.cos((r+this.colorsShift)*this.colorsScale)+1)/2;this.rainbowColors?this.ctx.strokeStyle=`hsl(${360*r}, 100%, 75%)`:this.ctx.strokeStyle=c.lerpColor(this.colorA,this.colorB,r),this.ctx.lineWidth=1,this.ctx.beginPath(),0===this.vertices&&c.pathCircle(this.ctx,a,s,h),1===this.vertices?c.pathCircle(this.ctx,a,s,1):c.pathPolygon(this.ctx,a,s,h,this.vertices,i*this.rotateShapes),this.ctx.stroke()}this.ctx.resetTransform()}getSettings(){return[{prop:"vertices",type:"int",min:0,max:8,toCall:"updateName"},{prop:"shapes",type:"int",min:0,max:2500},{prop:"rotateShapes",type:"bool"},{prop:"distanceRange",type:"float",min:0,max:this.distanceBase},{prop:"sizeRange",type:"float",min:0,max:this.sizeBase},{prop:"scale",type:"float",min:.05,max:1.95},{prop:"speed",type:"float",step:.1,min:-4,max:4},{prop:"colorsShift",type:"float",min:0,max:3.14},{prop:"colorsScale",type:"float",min:.05,max:2},{prop:"rainbowColors",type:"bool"}]}}e.exports=a},{"./animation":2,"./utils":27}],26:[function(t,e,i){"use strict";var s=t("./animation");const o=t("./utils");class a extends s{constructor(t,e,i,s=2500,a=2,r="random",h=!0,n=1){super(t,e,i,"spirograph","spirograph.js",`
