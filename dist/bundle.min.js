...
Coded with no external dependencies, using only canvas API.
`),this.lines=s,this.scale=a,this.speed=r,this.rainbowColors=h,this.radius=0}getVec(t){t=a.remap(t,0,this.lines,0,2*Math.PI);return a.rotateVec2d(a.createVec2d(this.radius,0),Math.PI+t)}draw(){this.clear(),this.radius=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/3*this.scale,this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2),a.strokeCircle(this.ctx,0,0,this.radius,this.colors[0]);for(let e=0;e<=this.lines;++e){var i=this.getVec(e),s=this.getVec(e*this.time);let t;t=this.rainbowColors?"hsl("+e/this.lines*360+", 100%, 75%)":a.lerpColorsPallet([this.colorA,this.colorB,this.colorA],e/this.lines),a.drawLine(this.ctx,i.x,i.y,s.x,s.y,t,1)}this.ctx.resetTransform()}getSettings(){return[{prop:"lines",type:"int",min:1,max:2500},{prop:"speed",type:"float",min:-2,max:2},{prop:"scale",type:"float",min:.25,max:1.75},{prop:"rainbowColors",type:"bool"}]}}e.exports=r},{"./animation":2,"./utils":25}],4:[function(t,e,i){"use strict";var s=t("./animation");const l=t("./noise"),c=t("./utils");class a extends s{constructor(t,e,i,s=180,a=.5,r=.4,h=1.2,n=.001,o=!1){super(t,e,i,"circular waves","circular-waves.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.particlesDensity=s,this.noiseScale=a,this.noise=n.noise,this.noise.seed(o.randomRange(0,1)),this.particlesSpeed=r,this.fadingSpeed=h,this.width=0,this.height=0,this.particles=[],this.imageData=null}update(t){this.time+=t/1e3,++this.frame;let e=1,i=this.particlesSpeed;for(;1<i;)i/=2,e*=2;for(var s of this.particles)s.prevX=s.x,s.prevY=s.y;for(let t=0;t<e;++t)for(var a of this.particles){var r=2*this.noise.perlin2(a.x*this.noiseScale,a.y*this.noiseScale)*Math.PI/this.noiseScale;a.x+=Math.cos(r)*a.speed*i,a.y+=Math.sin(r)*a.speed*i}}draw(){this.fadeOut(this.fadingSpeed);for(var t of this.particles)o.drawLine(this.ctx,t.prevX,t.prevY,t.x,t.y,t.color,2*t.radius);this.imageData=this.ctx.getImageData(0,0,this.ctx.canvas.width,this.ctx.canvas.height)}spawnParticles(e,i,s,a){var r=s*a*this.particlesDensity;for(let t=0;t<r;t++){var h=Math.random()*s+e,n=Math.random()*a+i;this.particles.push({x:h,y:n,prevX:h,prevY:n,speed:.2*Math.random()+.1,radius:.5*Math.random()+.5,color:o.randomChoice(this.colors)})}}reset(){this.clear(),this.particles=[],this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height,this.spawnParticles(0,0,this.width,this.height)}resize(){this.clear(),null!==this.imageData&&this.ctx.putImageData(this.imageData,0,0);var t=this.ctx.canvas.width-this.width,e=this.ctx.canvas.height-this.height;0<t&&this.spawnParticles(this.width,0,t,this.height),0<e&&this.spawnParticles(0,this.height,this.width,e),(0<t||0<e)&&this.spawnParticles(this.width,this.height,t,e),this.width=this.ctx.canvas.width,this.height=this.ctx.canvas.height;const i=this.width,s=this.height;this.particles=this.particles.filter(function(t){return!(t.x<0||t.x>i||t.y<0||t.y>s)})}getSettings(){return[{prop:"noiseScale",type:"float",step:.001,min:.001,max:.01,toCall:"reset"},{prop:"particlesDensity",type:"float",step:1e-4,min:1e-4,max:.002,toCall:"reset"},{prop:"particlesSpeed",type:"float",min:.25,max:32},{prop:"fadingSpeed",type:"float",step:1e-4,min:0,max:.01}]}}e.exports=a},{"./animation":2,"./noise":13,"./utils":25}],18:[function(t,e,i){"use strict";var s=t("./animation");const c=t("./utils"),d=t("./noise");class a extends s{constructor(t,e,i,s=1,a=.5,r=!1,h=.002,n={x:"random",y:"random",z:1},o=.01,l=!0){super(t,e,i,"quadtree visualization","quadtree.js",`
...
Coded with no external dependencies, using only canvas API.
`),this.shapeNames=["circles","points","lines","triangles","rectangles","pentagons","hexagons","heptagons","octagons"],this.vertices=this.assignIfRandom(a,c.randomInt(0,8)),this.updateName(),this.rotateShapes=r,this.shapes=s,this.distanceBase=.6,this.distanceRange=.2,this.sizeBase=.2,this.sizeRange=.12,this.scale=h,this.colorsScale=n,this.colorsShift=this.assignIfRandom(o,c.randomChoice([0,3.14])),this.rainbowColors=l}updateName(){this.name=this.shapeNames[this.vertices]+' "dancing" in a circle'}draw(){this.clear();var e=Math.max(this.ctx.canvas.width,this.ctx.canvas.height)/3*this.scale;this.ctx.translate(this.ctx.canvas.width/2,this.ctx.canvas.height/2);for(let t=0;t<this.shapes;++t){var i=t/this.shapes*2*Math.PI,s=(this.distanceBase+this.distanceRange*Math.cos(6*i+Math.cos(8*i+this.time/2)))*e,a=Math.cos(i)*s,s=Math.sin(i)*s,r=9*i-this.time,h=(this.sizeBase+this.sizeRange*Math.cos(r))*e,r=(Math.cos((r+this.colorsShift)*this.colorsScale)+1)/2;this.rainbowColors?this.ctx.strokeStyle=`hsl(${360*r}, 100%, 75%)`:this.ctx.strokeStyle=c.lerpColor(this.colorA,this.colorB,r),this.ctx.lineWidth=1,this.ctx.beginPath(),0===this.vertices&&c.pathCircle(this.ctx,a,s,h),1===this.vertices?c.pathCircle(this.ctx,a,s,1):c.pathPolygon(this.ctx,a,s,h,this.vertices,i*this.rotateShapes),this.ctx.stroke()}this.ctx.resetTransform()}getSettings(){return[{prop:"vertices",type:"int",min:0,max:8,toCall:"updateName"},{prop:"shapes",type:"int",min:0,max:2500},{prop:"rotateShapes",type:"bool"},{prop:"distanceRange",type:"float",min:0,max:this.distanceBase},{prop:"sizeRange",type:"float",min:0,max:this.sizeBase},{prop:"scale",type:"float",min:.05,max:1.95},{prop:"speed",type:"float",step:.1,min:-4,max:4},{prop:"colorsShift",type:"float",min:0,max:3.14},{prop:"colorsScale",type:"float",min:.05,max:2},{prop:"rainbowColors",type:"bool"}]}}e.exports=a},{"./animation":2,"./utils":25}],24:[function(t,e,i){"use strict";var s=t("./animation");const o=t("./utils");class a extends s{constructor(t,e,i,s=2500,a=2,r="random",h=!0,n=1){super(t,e,i,"spirograph","spirograph.js",`
